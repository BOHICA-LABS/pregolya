...
pass: A1
constraint: D16 Rust-blindness — record their choices as INFORMATIVE input to pregolya MAP
...
divergence from pregolya's Code Conventions (CLAUDE.md), because those divergences are
exactly the MAP decisions pregolya must make deliberately — not inherit.

## Toolchain baseline
- `edition = "2024"`, `resolver = "2"`, `rust-version = "1.94.0"`. Very recent MSRV
  (flagged in the manifest as not-yet-widely-deployed as of mid-2026). Pregolya pins its
...
## Async & core runtime
| Dep | Pin | Notes for pregolya MAP |
|-----|-----|--------------------------|
| tokio | 1.40, `default-features = false` | Matches pregolya's Tokio-multi-threaded, async-first convention. Default-features-off forces per-crate feature opt-in. |
| tokio-stream | 0.1 | Stream adapters. |
| async-trait | 0.1 | Every core trait is `#[async_trait]`. Pregolya must decide async-trait vs native `async fn in trait` (stable since 1.75) — adk chose the macro (object-safety for `dyn` trait objects, which native AFIT does not yet give without extra crates). This is a genuine MAP decision, not a default. |
...
| — | — | **No schemars.** adk builds JSON Schema by hand (`schema_utils`, `SchemaAdapter`). Pregolya's D5 mandates a pydantic→serde/schemars ADR; adk's hand-rolled approach is a data point *against* assuming schemars is required, but also a maintenance-cost data point. |
...
| anyhow | 1.0 | Present workspace-wide. Pregolya forbids `unwrap/expect` and mandates structured variants; anyhow in a *library* surface would conflict with that discipline. Deep pass must check whether anyhow leaks into public library signatures or is confined to binaries/tests. FLAG. |

## HTTP / TLS — **the sharpest divergence from pregolya conventions**
| Dep | adk pin | Pregolya rule | Disposition |
|-----|---------|-----------------|-------------|
| reqwest | 0.12, `default-features = false`, features `["json","stream","rustls-tls-native-roots","multipart"]` | `default-features = false, features = ["rustls-tls"]` + 30s timeout mandatory | adk correctly disables default-features (avoids native-tls) — GOOD and aligned. BUT it uses `rustls-tls-native-roots` (rustls backend reading the OS trust store) rather than `rustls-tls` (webpki-roots). Both are rustls crypto (no native-tls MITM path), so the security intent aligns; the root-store source differs. Pregolya must consciously pick `rustls-tls` vs `-native-roots`. Timeout enforcement is per-client, not visible in the workspace table — deep pass must verify 30s timeouts. |
| rustls | 0.23, features `["aws-lc-rs"]` | (crypto provider unspecified in conventions) | adk **unifies on the `aws-lc-rs` crypto provider** workspace-wide and installs it process-wide via `adk_core::ensure_crypto_provider()` (a `std::sync::Once` that calls `aws_lc_rs::default_provider().install_default()`, ignoring the result to respect a parent app's prior choice). Clean handling of the rustls "multiple providers" footgun. Pregolya should decide ring vs aws-lc-rs explicitly. |
| tokio-tungstenite | 0.28, `["rustls-tls-native-roots","connect"]` | — | WebSocket transport (OpenAI realtime ws_transport). rustls backend. |
| **livekit** | 0.7.36, `default-features = false`, features `["tokio","native-tls"]` | native-tls + aliases **forbidden workspace-wide** | **CONFLICT.** adk-realtime's LiveKit dependency pulls `native-tls`. Under pregolya's convention this is a hard violation (macOS Keychain init cost + MITM interception path). If pregolya ever ports realtime/LiveKit, this dep must be re-evaluated — LiveKit's rustls support (if any) or exclusion of the feature. FLAG for MAP. |
...
| Dep | Pin | Notes |
|-----|-----|-------|
| tracing | 0.1 | Structured logging throughout; `tracing::info_span!` with GCP-Vertex-style attributes in the runner (`gcp.vertex.agent.*`, `gen_ai.conversation.id`, `adk.*`). Aligned with pregolya's tracing convention. `adk-telemetry` re-exports `warn!`/etc. as a facade (`adk_telemetry::warn!` used in adk-model::retry). |
| tracing-subscriber | 0.3 | Subscriber config. |
| opentelemetry | 0.31 | OTel export via adk-telemetry. Pregolya's observability spec (Phase 1) should note adk uses OTel + gen_ai semantic conventions. |
...
## Summary of MAP-relevant flags (for pregolya architect, not conclusions)
1. **livekit → native-tls** is a hard conflict with pregolya's rustls-only rule. (HIGH)
2. **reqwest `rustls-tls-native-roots` vs pregolya's `rustls-tls`** — root-store choice diverges;
...
3. **anyhow present workspace-wide** — verify it does not leak into library public signatures
   (would violate pregolya's structured-error discipline). (MED — deep-pass verify)
4. **aws-lc-rs unified crypto provider + process-wide install-once** — a clean pattern pregolya
...
6. **async-trait everywhere** — object-safety choice; pregolya must pick async-trait vs native AFIT. (INFO)
7. **MSRV 1.94 / edition 2024** — adk rides the bleeding edge; pregolya pins its own. (INFO)
...
Recorded as INFORMATIVE input to pregolya MAP decisions (D16), not adoption evidence.

## Persistence / storage
| Dep | Where | Notes for pregolya MAP |
|-----|-------|--------------------------|
| sqlx | adk-graph (`sqlite` feature), adk-session (postgres/sqlite/…), adk-memory | Backends use `sqlx::{SqlitePool,PgPool}` with `pool.begin()`/`tx.commit()` transactions. Pregolya must pick sqlx vs sea-orm vs raw drivers, and decide whether graph checkpoints share the session DB layer (they don't in adk — P-27). |
...
| serde_json | cluster-wide | State/checkpoint serialization is `serde_json::to_string` of the whole state map — no msgpack, no allowlist gate (contrast LangGraph's ormsgpack + `SAFE_MSGPACK_TYPES` RCE guard, semport/graph §2.3). Pregolya's D5 serde/schemars ADR should note adk's plain-JSON choice AND the absence of a deserialization allowlist. |

## Cryptography (encryption-at-rest)
| Dep | Where | Notes |
|-----|-------|-------|
| aes-gcm | adk-session `encrypted.rs` | `Aes256Gcm` AEAD, `new_from_slice(&[u8;32])`. Correct AEAD choice. Pregolya's encryption-at-rest NFR (Phase 1) — data point for AES-256-GCM vs ChaCha20-Poly1305. |
| rand (`rng().fill_bytes`) | adk-session | 96-bit nonce generation. Pregolya must confirm a CSPRNG source; `rand::rng()` is thread-local. |
| base64 | adk-session | Envelope encoding of `[nonce‖ciphertext]`. |

## Delta / diff
| Dep | Where | Notes |
|-----|-------|-------|
| similar | adk-graph `delta.rs` (`delta-checkpoint` feature) | Character-level string diffs for `Diff for String`. Optional. Pregolya: only relevant if a string-delta checkpoint channel is ported. |

## Time / identity
| Dep | Where | Notes |
|-----|-------|-------|
| chrono | cluster-wide | `Utc::now()` drives checkpoint `created_at` and event `timestamp` — and these are the ORDERING keys (P-31). MAP flag: pregolya should not order history by wall-clock; use a monotonic per-thread sequence (LangGraph uses uuid6/logical versions). HIGH. |
...
choice. Reinforces the A1 async-trait-vs-native-AFIT MAP decision for pregolya (still INFO).

## Summary of new MAP-relevant flags (cluster; for pregolya architect, not conclusions)
1. **Wall-clock ordering of history** (checkpoint `created_at DESC`, session rewind by `timestamp`)
   — pregolya should use a monotonic logical sequence, not `Utc::now()`. (HIGH)
...
5. **sqlx transactions used correctly** for session multi-table atomicity — informative positive
   for the pregolya persistence layer's transaction discipline. (INFO)
6. **Two disjoint persistence dependency stacks** (graph checkpointer vs session service) — decide
   whether pregolya unifies them on one storage abstraction. (INFO)
...
pregolya MAP decisions; adk's choice is not evidence for adopting it.

## HTTP server & transport
| Dep | adk usage | Notes for pregolya MAP |
|-----|-----------|--------------------------|
| axum | server framework | `Router`, `middleware::from_fn`, `DefaultBodyLimit`, extractors. `tower`/`tower-http` supply `CorsLayer`, `TimeoutLayer`, `SetResponseHeaderLayer`, `TraceLayer`. Mature HTTP stack. Pregolya-server (D13, first-party) makes its own choice; adk is a reference for middleware composition (patterns A3 P-36). |
...
`TimeoutLayer` does not bound outbound calls. **Disposition: HARD divergence from pregolya's
mandatory-30s-timeout rule (patterns A3 P-42).** Pregolya must set `.timeout()` on ALL outbound
...
|---------|----------|-------------|
| `anyhow` NOT in any exposure-cluster library src | grep: 0 hits in `adk-server`/`adk-auth`/`adk-awp`/`adk-acp`/`awp-types`/`adk-telemetry`/`adk-managed`/`adk-enterprise` `src/`; declared in `adk-server`/`adk-deploy` Cargo.toml but unused in src; USED only in `adk-cli` + `cargo-adk` (binaries) | **A1 P-18 RESOLVED for this cluster** — anyhow is confined to binaries, exactly pregolya's permitted carve-out; no library public signature leaks it. With A2's clean state-cluster finding, only the core-crate grep remains (out of scope). (RESOLVED) |
| Structured boundary errors | `A2aError`, `RequestContextError`/`AuthError`/`AccessDenied` are `thiserror`-derived; server maps `AdkError.category` → HTTP status | Aligned with pregolya structured-error discipline. (INFO) |

## Credential handling — divergence flag
| API | adk choice | Pregolya rule | Disposition |
|-----|-----------|-----------------|-------------|
| `SecretProvider::get_secret -> Result<String, AdkError>` | bare `String` (default Debug/Display) | newtype + redacted `Debug` mandatory | **Divergence (patterns A3 P-44).** Pregolya's secret-service analog must return a redacted newtype. (MED) |
| Cloud secret backends | `aws-sdk-secretsmanager`/Azure Key Vault/GCP Secret Manager behind features | — | Reference for a pluggable secret-provider seam; pregolya would wrap results in redacted newtypes. (INFO) |

## Protocol dependencies (new in this cluster)
| Dep | Crate | Role | pregolya relevance |
|-----|-------|------|----------------------|
...
| `sha2`/`hex` | a2a push / AWP | HMAC-SHA256 webhook signing | Reference for signed-webhook delivery if pregolya-server adds webhooks. |

## Observability
| Dep | adk usage | pregolya relevance |
|-----|-----------|----------------------|
| `opentelemetry` (0.31) + `adk-telemetry` | `AdkSpanExporter` wired into `ServerConfig.span_exporter`; `semconv` exposes `gen_ai.*` OTel constants (request/usage/response) | adk uses OTel + gen_ai semconv end-to-end into the server. Pregolya's observability spec (Phase 1) can reference `gen_ai.usage.*` for token telemetry. Token metering is telemetry-only — no budget gate (§16 / P-46). (INFO) |
...
1. **Outbound reqwest clients have NO timeout cluster-wide** (a2a client/push, JWKS, OIDC). HARD
   divergence from pregolya's 30s rule. (HIGH) — RESOLVES A1 open item.
...
4. **axum + tower/tower-http middleware stack** — clean reference for pregolya-server's inbound
   security posture (SSRF gate, security headers, request-id, body limit). (INFO)
5. **A2A/ACP/AWP each pull an external protocol dep** — all out of pregolya's declared scope (D1). (INFO)
...
D16 Rust-blindness: adk's choices are INFORMATIVE input to pregolya MAP decisions, not evidence
for adoption. Cross-refs P-47..P-66.

## Isolation / sandbox dependencies
| Dep | adk crate | Purpose | pregolya relevance |
|-----|-----------|---------|----------------------|
| `wasmtime` 45 + `wasmtime-wasi` 44 | adk-sandbox (feature `wasm`), adk-code (wasm_guest) | In-process WASM isolation | STRONGEST isolation primitive in corpus (P-47). If pregolya wants a default-isolating code backend, WASM (wasmtime) is the reference. Heavy dep; feature-gate it. |
...
| `tempfile` 3 | adk-sandbox, adk-code | Temp dirs for source/binary | Fine; pregolya uses same pattern. |
| `windows-sys` 0.59 | adk-sandbox (feature `sandbox-windows`) | AppContainer Win32 APIs | Platform enforcer; pregolya Windows sandbox would need equivalent. |
...
| (external binaries) `bwrap`, `sandbox-exec` | adk-sandbox OS enforcers | Runtime-probed, not deps | NOTE: Linux/macOS enforcers depend on external binaries present at runtime — not a cargo dep but a deploy-time requirement. pregolya must handle absence gracefully (adk does, via `probe()`). |
...
| `proptest` (dev) | adk-eval, adk-code, adk-sandbox, adk-retry-reflect | Property tests | Aligns with pregolya VP/property-test discipline (Phase 6). |

## Skill / plugin / browser dependencies
| Dep | adk crate | Purpose | pregolya relevance |
|-----|-----------|---------|----------------------|
| `serde_yaml` 0.9 | adk-skill | Skill frontmatter parsing | serde_yaml is UNMAINTAINED upstream (archived). If pregolya adopts a SKILL.md model, prefer a maintained YAML crate (e.g. `serde_yml`/`yaml-rust2`). (MED) |
...
| `thirtyfour` 0.35 | adk-browser | WebDriver client | Browser automation via WebDriver (Selenium). Out of pregolya's declared scope; noted. NO reqwest in this crate. |
...
1. **`wasmtime` is the reference isolation dep** (P-47) — feature-gate, but it is the default-safe
   code-exec backend pregolya should consider. (INFO/HIGH)
2. **`serde_yaml` is unmaintained** — if pregolya adopts SKILL.md parsing, choose a maintained
   YAML crate. (MED)
3. **OS sandbox enforcers depend on external runtime binaries** (`bwrap`/`sandbox-exec`), probed not
   linked — pregolya must probe + degrade gracefully. (INFO)
...
anyhow verdict** (A1 open item P-18). D16 Rust-blindness — informative to pregolya MAP, not
adoption evidence.
...
entry in adk-model. Pregolya forbids even the single `Other(anyhow)` variant on a public error enum;
...
- **Ollama** (`adk-model` `ollama` feature → `ollama-rs`): an HTTP client to a local Ollama daemon.
  ...
  the shape pregolya-ollama targets. Keyless-CI-friendly: the HTTP boundary is mockable (wiremock),
  and the reusable insight is the text-tag `tool_call_parser` (P-68) for Ollama models lacking native
  tool-calling. `ollama-rs` owns timeout — pregolya would set its own.
- **mistral.rs** (`adk-mistralrs` → `mistralrs 0.8` / candle): IN-PROCESS native inference. Broad
  ...
  but real inference needs GB-scale cached weights + a heavy toolchain. Relevance to pregolya:
...
adk-mistralrs/adk-audio without ML-download features) avoid native-tls. **Disposition:** any pregolya
port of local inference or LiveKit voice inherits a native-tls conflict with the rustls-only rule; the
`hf-hub`→native-tls chain in particular gates BOTH local-LLM and audio-ML. Pregolya must resolve via
...
| API | adk choice | Pregolya rule | Disposition |
|-----|-----------|-----------------|-------------|
| all provider configs + SDK clients | `pub api_key: String` on `#[derive(Debug,…)]` (several also `Serialize`) | newtype + redacted `Debug` | **Divergence workspace-wide (P-76).** No redacted newtype anywhere; even `adk-anthropic::Anthropic` derives Debug over its `api_key`. Pregolya must wrap every key. (MED→ pervasive) |
...
timeout-less (P-77). Pregolya must set `.timeout()` on every provider client — the anthropic main
...
**MAP consequence:** the livekit native-tls ingress is the only one a pregolya realtime-analog
