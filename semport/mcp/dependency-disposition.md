...
**Decision: MAP the `mcp` Python SDK to `rmcp` (official).** `ferrochain-mcp`
becomes an adapter from rmcp's client types to ferrochain-core's tool/message
...
| Command result (langgraph) | optional | N/A (ferrochain-graph owns this) | route to ferrochain-graph |

## Other dependencies

| Python dep | Used for | Disposition | Rust target | Notes |
|---|---|---|---|---|
| `langchain-core` | `BaseTool`/`StructuredTool`, `ToolMessage`, content blocks, `Blob`, messages | PORT (internal) | `ferrochain-core` | The whole point is producing ferrochain-core tool/message types. Depends on core semport landing tool + content-block types first. |
...
| `langgraph` (optional) | `Command` tool-result variant + runtime | ROUTE | `ferrochain-graph` | `Command` result handling belongs to ferrochain-graph; feature-gate `ferrochain-mcp`'s Command support behind a `graph` feature that depends on ferrochain-graph. |
...
- **Auth headers / bearer tokens** transiting the HTTP client are credentials →
  if ferrochain-mcp wraps any API key in a config type, apply the redacted
  newtype `Debug` rule.
...
- **MAP (HIGH confidence):** `mcp` → **rmcp 2.2.0 (official)**; `httpx` →
  `reqwest` (rustls-tls); langchain-core tool/message/Blob → ferrochain-core.
...
- **ROUTE:** langgraph `Command` result → ferrochain-graph feature.
