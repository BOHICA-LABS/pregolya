...
project: ferrochain
port_target: langgraph-sdk @ 1.2.9 + langgraph-cli @ 1.2.9 → ferrochain-platform-client (+ ferrochain-cli)
analyzer_pass: 6
date: 2026-07-12
...
consistency: aligns with semport/partners/rust-translation-strategy.md §1 (the
      ferrochain-partner-http direct-HTTP pattern applies here) and semport/graph
...
1. `ferrochain-platform-client` — a Rust client for a LangGraph-Server-compatible API. Only
   valuable if ferrochain (a) wants interop with existing LangGraph deployments, or (b)
   ships its own hosted server that this client talks to. **Gated on D9/D11 server decision.**
2. `ferrochain-cli` — a project CLI. Only the `validate` + config-schema slice ports; the
...
## 1. `ferrochain-platform-client` shape — 🟠

### 1.1 Reuse ferrochain-partner-http (the key consistency win)
The SDK's transport needs are a SUBSET of what `ferrochain-partner-http` (semport/partners
...
on `ferrochain-partner-http`**, not re-implement transport. Divergences to add there or
layer on top:
- **read timeout override**: the SDK uses `read=300s` for long runs; expose a per-request
  timeout override (ferrochain default 30s must NOT apply to `runs.wait`/`join`/stream).
...
```rust
// illustrative sketch
pub struct PlatformClient { http: ProviderClient /* from ferrochain-partner-http */, base_url: Url }
impl PlatformClient {
...
### 1.7 Error taxonomy — 🟡
PORT `errors.py` status→variant map into the ferrochain error taxonomy
...
`ferrochain-graph` implementing the same `Runnable`/graph trait the local `Pregel` exposes,
delegating to `ferrochain-platform-client`. Mirrors `langgraph.pregel.remote.RemoteGraph`:
...
Gated on: (a) `ferrochain-platform-client` existing, (b) the D9/D11 server strategy. This is
the ONLY platform component with a clean parity argument to the local engine — but still a
late, optional wave.

## 3. `ferrochain-cli` — the portable slice only — 🟢/🟡
...
The `schemas.Config` TypedDict → a ferrochain project-config struct (name TBD:
`ferrochain.toml` preferred over JSON for Rust ecosystem fit; support both via serde).
...
```rust
// illustrative sketch — ferrochain project config
#[derive(Deserialize)] #[non_exhaustive]
pub struct FerrochainConfig {
...
ferrochain graph crates. DEFER to a DX wave.

### 3.4 Everything else — DROP
`up`/`dev`/`build`/`dockerfile`/`deploy` are Docker/Python-packaging/SaaS-bound. A future
ferrochain `dev`/`serve` command would launch a ferrochain server binary (net-new, gated on
...
|---|---|---|---|
| ferrochain-partner-http reuse for transport | 🟢 | build-on | read-timeout override for long runs |
...
| ferrochain.toml schema | 🟡 | PORT | which fields survive Python→Rust |
| `validate` cmd | 🟢 | PORT | — |
| CLI up/dev/build/deploy | — | DROP | Docker/packaging/SaaS, no port value |

## 5. Top open design questions (feeds architect / P1-06)

1. **Build the platform client at all?** Gated on D9/D11: is there a hosted ferrochain
...
3. **Transport crate boundary**: does `ferrochain-partner-http` grow the resumable-SSE +
   Location-follow + long-read-timeout features, or does a `ferrochain-platform-transport`
   layer sit on top? (Recommend: extend partner-http; the SSE reconnect is reusable.)
4. **`ferrochain.toml` vs `ferrochain.json`**: config format + which `langgraph.json` fields
