...
| `test_channels.py` | 803 | Per-channel-type behavioral contract (LastValue single-write error, BinOp fold+Overwrite, Topic accumulate, barrier availability, ephemeral lifetime, untracked exclusion). | P0 — channel algebra |
...
| `checkpoint/tests/test_jsonplus.py` (1,237) | | **serialization round-trip golden spec** — every typed object (datetime/uuid/decimal/set/deque/ip/path/messages/langgraph types), msgpack + jsonplus fallback, strict allowlist. MUST be byte-faithful in Rust. |
