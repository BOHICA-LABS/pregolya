...
### AC-001 (traces to BC-2.05.001 PC-001 — interrupt() suspends run and writes INTERRUPT marker)
...
### AC-002 (traces to BC-2.05.001 PC-003 — INTERRUPT marker uses sync put_writes)
...
### AC-006 (traces to BC-2.05.002 INV-002 — interrupt_counter tracks multiple interrupts in same task)
...
### AC-007 (traces to BC-2.05.002 INV-003 — scratchpad is per-task, not per-run)
...
### AC-008 (traces to BC-2.05.003 PC-002 — node re-executes from START on resume)
...
### AC-009 (traces to BC-2.05.003 PC-004 — resume value replayed from scratchpad)
...
### AC-010 (traces to BC-2.05.003 INV-002 — idempotent replay from scratchpad)
...
### AC-011 (traces to BC-2.05.004 PC-001 — Command struct has resume, update, goto, graph fields)
...
### AC-012 (traces to BC-2.05.004 INV-003 — Command.PARENT shorthand maps to parent run)
...
### AC-013 (traces to BC-2.05.004 EC-004 — Command on non-interrupted run returns E-GRAPH-002)
...
### AC-014 (traces to BC-2.05.004 PRE-004 — interrupt_id field is canonical in Command)
...
### AC-015 (traces to BC-2.05.005 PC-001 — empty resume queue returns E-GRAPH-002)
...
### AC-016 (traces to BC-2.05.005 PRE-002 — 7 guard cases on run state)
...
### AC-017 (traces to BC-2.05.005 PC-003 — HTTP 422 for invalid resume)
...
### AC-018 (traces to BC-2.05.005 INV-003 — queue exhausted vs wrong state are distinct error payloads)
...
### AC-019 (traces to BC-2.05.006 INV-002 — ActionRisk enum defined with 4 variants)
...
### AC-020 (traces to BC-2.05.006 INV-002 — wildcard arm fails-closed to High for unknown variants)
...
### AC-021 (traces to BC-2.05.006 PC-003 — risk gate triggers interrupt for configured threshold)
...
### AC-022 (traces to BC-2.05.006 EC-001 — E-GRAPH-013 on risk gate rejection)
