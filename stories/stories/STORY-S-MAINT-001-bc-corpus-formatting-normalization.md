...
- **So that** automated validators, grep patterns, and human reviewers can rely on
  consistent structure across all 140 BCs without format-conditional logic or
...
- **`## Invariants` section:** 94 of 140 BCs use bullet lists; 40 use numbered lists.
  Affected BC families: BC-2.04.*, BC-2.11.*, BC-2.13.*, BC-2.18.*, BC-2.19.*,
  BC-2.20.*, BC-2.21.*, BC-2.22.*.
- **`## Edge Cases` section:** 81 of 140 BCs use `### EC-NNN` subsection headers;
...
### AC-002: All 140 `## Invariants` sections use the chosen canonical format
...
### AC-003: All 140 `## Edge Cases` sections use the chosen canonical format
After normalization, `grep -rn` across `.factory/specs/behavioral-contracts/`
...
a total-BC count of 140. No AC-to-BC trace is broken by the formatting change.

### AC-005: BC-INDEX.md content and row count unchanged
`BC-INDEX.md` `Total BCs: 140` count is unchanged. No BC ID, title, priority,
...
| BC corpus (140 files, ~300 tokens avg) | ~42,000 |
...
- [ ] Run `verify-ac-pc-trace.sh` and confirm exit 0 with count=140
...
|---------|------|--------|--------|
| 1.3 | 2026-09-02 | story-writer | Phase-2 gate consistency-audit C-2 (human-approved): BC-corpus count references updated 134→140 (CAP-040 added BC-2.02.007/008/009 and BC-2.04.009/010/011); input-hash refreshed to match current BC-INDEX.md per drift-detection rule. |
