...
changelog:
  ...
  - "1.9 (round-49/BC-propagation/2026-08-31): BC-2.09.007 {INV-003} extended from 4 to 6 patterns (URL-userinfo + HTTP Basic). AC-017 step 2: 'canonical four-pattern set' → 'canonical six-pattern set'; patterns (e) URL-embedded userinfo `[a-zA-Z][a-zA-Z0-9+.\\-]*://[^/\\s:@]+:[^/\\s:@]+@` (TV-007 scope extended) and (f) HTTP Basic auth `Basic\\s+[A-Za-z0-9+/=]+` (base64 padding prevents pattern (c) match) added. Architecture Compliance Rule 11 and SSE handler Task updated to cite 6-pattern set. input-hash refreshed (b5934d3 — BC-2.12.007 round-49 computed hash)."
phase: 2
inputs:
...
Before `StreamEvent::Error.error_message` is emitted on the SSE surface (`pregolya-server/src/sse.rs`) and before any unary non-2xx error body is returned, the `pregolya-server` SSE handler MUST apply the mandatory 3-step SEC-BOUND-001 sanitization pipeline in exact order (per BC-2.12.007 {INV-004}): (1) **internal-panic static-replace** (per BC-2.12.007 {INV-004} step 1): if `error.code ∈ {"E-GRAPH-011", "E-GRAPH-019"}`, replace the error message with the corresponding STATIC message — no dynamic panic text, no `source_node` topology (E-GRAPH-011 STATIC: `"ConditionalEdgePanic: conditional edge function panicked during execution — see server error log for details"`; E-GRAPH-019 STATIC: `"NodePanic: graph node panicked during execution — see server error log for details"`); (2) **`redact_credentials`**: apply the canonical six-pattern set (per BC-2.12.007 {INV-004} step 2) — replace each match with `"<redacted>"`: `sk-[A-Za-z0-9_\-]{20,}`, `sk-ant-[A-Za-z0-9_\-]{32,}`, `[A-Za-z0-9]{64,}`, `Bearer\s+[A-Za-z0-9._~+/=\-]+`, `[a-zA-Z][a-zA-Z0-9+.\-]*://[^/\s:@]+:[^/\s:@]+@` (URL-embedded userinfo), `Basic\s+[A-Za-z0-9+/=]+` (HTTP Basic auth; base64 padding `+`/`=` prevents pattern 3 from matching); (3) **`sanitize_internal_ids`**: replace UUID-shaped internal identifiers with `"<redacted-id>"`; `u64` CheckpointId carve-out: NOT UUID-shaped, NOT covered by this pass. No step may be skipped or reordered (ADR-029 SEC-BOUND-001). Test vectors: TV-007 (E-GRAPH-011 static-replace on SSE surface); TV-008 (E-GRAPH-019 static-replace on SSE surface); TV-009 (Bearer-token credential redaction on SSE surface).
...
- [ ] Implement SEC-BOUND-001 sanitization pipeline on SSE handler (`pregolya-server/src/sse.rs`) and unary non-2xx error path — apply 3-step pipeline in mandatory order before emitting `StreamEvent::Error.error_message` or returning a non-2xx error body: (1) E-GRAPH-011/E-GRAPH-019 static-replace per BC-2.12.007 {INV-004} step 1; (2) `redact_credentials` 6-pattern set — OpenAI key `sk-*`, Anthropic key `sk-ant-*`, generic long alphanumeric `[A-Za-z0-9]{64,}`, Bearer token, URL-embedded userinfo, HTTP Basic auth `Basic\s+...` (replace each with `<redacted>`); (3) `sanitize_internal_ids` UUID-only (replace with `<redacted-id>`); verify via BC-2.12.007 TV-007/TV-008/TV-009 (AC-017)
