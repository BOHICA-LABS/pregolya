...
changelog:
  ...
  - "1.29 (round-41/F-P2A173-01+F-P2A175-01/2026-08-29): AC-033 node-body-panic coverage extended (F-P2A173-01 [LOW]): BC-2.09.008 EC-010 scope note propagated — FutureExt::catch_unwind(AssertUnwindSafe(runner.run(...))) covers ALL panics during .await polling, including graph-node-body panics (higher-probability CWE-248 vector per BC-2.09.008 EC-010) as well as extract_output panics; AC-033 heading/body updated; EC-012 scenario extended; Task-1/Task-36/Task-37 AC-033 descriptions updated; Arch-Compliance EC-010 row updated. AC-020 result_text branching NO-OP (F-P2A175-01 [LOW]): existing AC-020 already carries branching rule Value::String→verbatim; other variants→serde_json::to_string(&extract_output_result) per BC-2.09.007 {PC-002}; universal-gloss finding not confirmed. input-hash updated (BC-2.09.008 §Changelog; round-41)."
  - "1.30 (round-42/F-P2A179-01+F-P2A177-02/2026-08-29): F-P2A179-01 [HIGH]: AC-019 ConcreteGraphRunner<S> / 'where S is statically known' replaced with non-generic canon per BC-2.09.008 {PC-003} v3.4 / ADR-029 §Decision 2: ConcreteGraphRunner::run (non-generic, no <S>; holds Arc<CompiledStateGraph> + Box<dyn Fn(&serde_json::Value) -> serde_json::Value + Send + Sync>). F-P2A177-02 [MED, CWE-209]: AC-019 extended with EC-003 static-replace exception clause — E-GRAPH-011 ConditionalEdgePanic and E-GRAPH-019 NodePanic are EXCEPTIONS to the original-error-code propagation rule; at MCP boundary the server substitutes content[0].text = 'internal error' (no panic text, no dynamic fields) per ADR-029 §Decision 5. AC-038 added: TV-019 path_fn conditional-edge panic → E-GRAPH-011 → static 'internal error' via FutureExt::catch_unwind(AssertUnwindSafe(runner.run(...))). E-GRAPH-019 added to Arch-Compliance EC-003 row. Body §Changelog entry added. input-hash refreshed (BC-2.09.008 updated round-42)."
...
- If schema validation passes, `GraphAgentTool::invoke_dyn` delegates to
  ...
  **Exception (ADR-029 §Decision 5 static-replace routing table):** When the original error
  carries an internal-panic code — specifically `E-GRAPH-011 ConditionalEdgePanic` (path_fn
  panicked during conditional edge routing) or `E-GRAPH-019 NodePanic` (node body panicked
  during execution) — the MCP server applies STATIC replacement instead of forwarding the
  redacted `PregolyaError::message`. The response is `isError: true` with
  `content[0].text == "internal error"` (no panic text, no `source_node`, no captured message
  forwarded). These codes carry non-static messages with dynamic fields that would expose
  internal implementation details at the MCP boundary even after credential redaction; static
  replacement is the correct defense (CWE-209). All other graph error codes continue to have
  their redacted `PregolyaError::message` forwarded normally. See AC-038 for the E-GRAPH-011
  conditional-edge-panic test path (TV-019).
...
BC-2.09.007 {PC-002}: `Value::String(s)` → `result_text = s` verbatim; all other `Value`
variants → `result_text = serde_json::to_string(&extract_output_result)` (compact JSON).
...
### AC-033 (traces to BC-2.09.008 EC-010 — panic recovery in `invoke_dyn`, Red Gate)
**Red Gate / ALL panics during `.await` polling caught via `FutureExt::catch_unwind` inside
`invoke_dyn` — static response (covers graph-node-body panics + `extract_output` panics):**
The recovery mechanism MUST be
`futures::future::FutureExt::catch_unwind(AssertUnwindSafe(runner.run(input, policy)))`
applied INSIDE `GraphAgentTool::invoke_dyn` at the awaited call site. This wrap covers ALL
panics that occur during `.await` polling of `runner.run(input, policy)` — including:
(a) **graph-node-body panics**: a panic inside a graph node body during execution (a
higher-probability CWE-248 vector than `extract_output` panics, because graph node code is
typically user-authored and runs throughout graph execution); and (b) **`extract_output`
panics**: a panic in the caller-supplied `extract_output` closure after successful graph
completion. Per BC-2.09.008 EC-010, both categories produce the same outcome. Synchronous
`std::panic::catch_unwind` is INADEQUATE for this path — because both panic sources fire
during `.await` polling, a synchronous catch placed outside the async call chain does not
intercept the panic. The response MUST be:
...
This is a Red Gate: without `FutureExt::catch_unwind` wrapping, a panicking graph-node body or
`extract_output` closure propagates through the async runtime, crashing the server process
(remote DoS, CWE-248). See also AC-037 for the SEC-008 `panic = "unwind"` build-profile
prerequisite.
...
(e.g., a `[profile.release] panic = "unwind"` line inside `pregolya-mcp/Cargo.toml` itself)
is silently ignored by the Cargo linker and MUST NOT be relied upon as the SEC-008 gate.
The `catch_unwind` boundary lives in the `pregolya-server` binary request handler — it is the
`pregolya-server` binary's workspace-root profile setting that determines the effective panic
mode at runtime. If the workspace-root release profile sets `panic = "abort"`,
...
denial-of-service (CWE-248). This is a Phase-3 devops-engineer obligation (workspace-root
`Cargo.toml` authoring; assert on the `pregolya-server` binary profile, NOT the
`pregolya-mcp` library-member profile). The pregolya-mcp story-level obligation: the
implementing engineer MUST add a
`// SEC-008: panic = "unwind" required in workspace-root release profile (pregolya-server) — library-member profile is inert; FutureExt::catch_unwind voids under abort`
comment in `pregolya-mcp/Cargo.toml` or at the `invoke_dyn` call site to document the
dependency. The devops-engineer asserts the workspace-root profile at Phase-3 workspace init.
...
| EC-012 | `GraphAgentTool`: panic during `.await` polling in `invoke_dyn` — graph-node-body panic (higher-probability CWE-248 vector) OR `extract_output` panic after successful graph completion | `isError: true`, `content[0].text == "internal error"` (static); server continues serving subsequent requests — BC-2.09.008 EC-010 |
...
| Test files (~230 lines; AC-001–AC-038 + 13 Red Gates) | ~3,300 |
| Tool outputs | ~600 |
| **Total** | **~24,850** |
...
36. [ ] **Red Gate check (AC-033):** confirm `test_BC_2_09_008_ec010_extract_output_panic_caught_unwindsafe_static_response()` FAILS when only synchronous `std::panic::catch_unwind` is used (unhandled panic crashes the handler because a panic during `.await` polling — whether in the `extract_output` closure or a graph-node body — is NOT intercepted by a synchronous catch placed outside the async call chain; no `isError` response is produced — BC-2.09.008 EC-010)
...
| ALL panics during `.await` polling (graph-node-body panics + `extract_output` panics + conditional-edge path_fn panics) caught via `futures::future::FutureExt::catch_unwind(AssertUnwindSafe(runner.run(input, policy)))` INSIDE `GraphAgentTool::invoke_dyn` at the `.await` call site; synchronous `std::panic::catch_unwind` is INADEQUATE (panic occurs during `.await` polling); response is static `"internal error"` (`isError: true`); `E-GRAPH-011 ConditionalEdgePanic` and `E-GRAPH-019 NodePanic` are internal-panic codes subject to ADR-029 §Decision 5 static-replace (NOT normal error forwarding); server continues serving; SEC-008: workspace release profile MUST pin `panic = "unwind"` | BC-2.09.008 EC-010; EC-003 static-replace exception; SEC-008 | Tests AC-033 Red Gate, AC-037 build obligation, AC-038 TV-019 |
...
- **1.29 (round-41 / F-P2A173-01 + F-P2A175-01 / 2026-08-29):** (1) F-P2A173-01 [LOW] AC-033 node-body-panic coverage: BC-2.09.008 EC-010 scope note propagated. `FutureExt::catch_unwind(AssertUnwindSafe(runner.run(input, policy)))` covers ALL panics during `.await` polling — including graph-node-body panics (higher-probability CWE-248 vector per BC-2.09.008 EC-010, because graph node code is typically user-authored and runs throughout graph execution) as well as `extract_output` panics; both categories produce the same static `"internal error"` isError:true response. AC-033 heading updated from "extract_output panic recovery" to "panic recovery in `invoke_dyn`"; AC-033 body extended with explicit (a) graph-node-body panic category and (b) `extract_output` panic category, citing BC-2.09.008 EC-010 as the scope authority. EC-012 edge-case scenario updated: "extract_output closure panics" expanded to "panic during `.await` polling — graph-node-body panic OR extract_output panic". Task-1 AC-033 description extended from "extract_output panic" to "all panics during `.await` polling — graph-node-body + extract_output". Task-36 Red Gate check description extended to note both panic sources. Task-37 implementation description extended to name both panic categories and their identical outcome. Architecture Compliance EC-010 row updated from "extract_output panic" to "ALL panics during `.await` polling (graph-node-body panics + extract_output panics)". (2) F-P2A175-01 [LOW] AC-020 result_text branching NO-OP: verification confirms existing AC-020 already carries the correct branching rule — `Value::String(s)` → `result_text = s` verbatim; all other `Value` variants → `result_text = serde_json::to_string(&extract_output_result)` (compact JSON) per BC-2.09.007 {PC-002}; the universal-serde_json gloss finding is not confirmed in AC-020; no edit applied. input-hash updated (BC-2.09.008 §Changelog; round-41).
- **1.30 (round-42 / F-P2A179-01 + F-P2A177-02 / 2026-08-29):** Two round-42 findings closed. (1) F-P2A179-01 [HIGH] — AC-019 `ConcreteGraphRunner<S>::run` / "where `S` is statically known" replaced with non-generic canon per BC-2.09.008 {PC-003} v3.4 and ADR-029 §Decision 2: `ConcreteGraphRunner::run` (non-generic, no `<S>`; holds `Arc<CompiledStateGraph>` + `Box<dyn Fn(&serde_json::Value) -> serde_json::Value + Send + Sync>`). Exhaustive sweep confirmed this was the sole live-body `ConcreteGraphRunner<S>` / "where S is statically known" occurrence. (2) F-P2A177-02 [MED, CWE-209] — AC-019 extended with EC-003 static-replace exception clause per BC-2.09.008 {PC-003} EC-003 amendment: `E-GRAPH-011 ConditionalEdgePanic` and `E-GRAPH-019 NodePanic` are EXCEPTIONS to the "original graph error code is propagated" rule; at the MCP boundary the server substitutes `content[0].text = "internal error"` (static replacement; no panic text, no `source_node`, no dynamic fields forwarded) per ADR-029 §Decision 5 routing table; all other graph error codes continue to have their redacted `PregolyaError::message` forwarded normally (CWE-209). AC-038 added: TV-019 path — conditional-edge path_fn panic → `E-GRAPH-011 ConditionalEdgePanic` caught via `FutureExt::catch_unwind(AssertUnwindSafe(runner.run(...)))` → static `"internal error"` at MCP boundary; verified by `test_BC_2_09_008_ec003_conditional_edge_panic_yields_static_internal_error_tv019()`. Architecture Compliance EC-010 row updated to name all three panic categories (graph-node-body, extract_output, conditional-edge path_fn) and cite EC-003 static-replace exception with E-GRAPH-011/E-GRAPH-019. input-hash refreshed (BC-2.09.008 updated round-42).
