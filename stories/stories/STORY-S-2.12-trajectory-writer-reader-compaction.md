...
After any storage I/O error during `compact`, `replay(run_id)` returns the exact same result as before the attempted compaction — no partial compaction state is observable. The staging-table swap model (see AC-020 and Architecture Rule 4) ensures reader-visible atomicity: a reader sees either the complete pre-compaction trajectory or the complete post-compaction trajectory, never a partially applied state. Verified by `test_BC_2_04_011_compact_error_leaves_trajectory_intact()`.

### AC-020 (traces to BC-2.04.011 PC-004 / INV-003 — reader-visible atomicity; staging-table swap; SIGKILL at any crash point leaves pre-compaction trajectory intact — RED GATE — VP-019 anchor)
A process kill (SIGKILL) at any point during compaction execution leaves the pre-compaction trajectory fully intact. After restart, `replay(run_id)` returns the exact pre-compaction record set. The implementation uses a two-phase staging-table swap under WAL mode (`PRAGMA journal_mode=WAL`): (1) **Build phase** — populate `trajectory_records_staging` with the post-compaction retained record set using bounded batches (default 1,000 records per `BEGIN IMMEDIATE` / `COMMIT`); (2) **Swap phase** — a single brief `BEGIN IMMEDIATE` / COMMIT that renames `trajectory_records_staging` to `trajectory_records` (atomic SQLite rename). VP-019 covers all four crash points: (1) before-build-begins — `trajectory_records` intact, no staging table exists; (2) mid-build staging partially filled, before swap — `trajectory_records` intact, staging table dropped on recovery; (3) mid-swap-transaction after `BEGIN IMMEDIATE` before `COMMIT` — WAL mode discards uncommitted frames on next open, `trajectory_records` restored to pre-swap state; (4) after-swap-commit — full post-compaction state permanently visible. This test MUST fail on stubs before implementation (Red Gate discipline — a stub lacking the staging-table swap is not crash-safe). Verified by `test_BC_2_04_011_crash_isolated_compaction()` (VP-019 integration test anchor; uses a temp SQLite file).
...
| `TrajectoryCompactor` trait | `pregolya_checkpoint::trajectory` (`checkpoint::trajectory`) | pregolya-checkpoint | Effectful Shell (two-phase staging-table swap: build phase uses bounded `BEGIN IMMEDIATE` / `COMMIT` batches on `trajectory_records_staging`; swap phase issues single `BEGIN IMMEDIATE` / rename / `COMMIT` under WAL mode) |
...
| `SqliteTrajectoryStore::compact` | Effectful Shell | Two-phase staging-table swap via `rusqlite`: build phase populates `trajectory_records_staging` in bounded `BEGIN IMMEDIATE` / `COMMIT` batches (WAL mode); swap phase issues single `BEGIN IMMEDIATE` / rename `trajectory_records_staging` → `trajectory_records` / `COMMIT`; crash-safe atomic mutation |
...
- [ ] Implement `SqliteTrajectoryStore::compact` using two-phase staging-table swap with WAL mode (`PRAGMA journal_mode=WAL`): build phase creates `trajectory_records_staging` and populates it with the retained record set using bounded batches (default 1,000 records per `BEGIN IMMEDIATE` / `COMMIT`); swap phase issues a single brief `BEGIN IMMEDIATE` / rename `trajectory_records_staging` → `trajectory_records` / `COMMIT`; any storage I/O failure at any phase → `Err(PregolyaError { category: DURABILITY, code: "E-TRAJ-005" })` with no partial compaction state observable (Architecture Rule 4)
...
4. `TrajectoryCompactor::compact` MUST use a two-phase staging-table swap with WAL mode (`PRAGMA journal_mode=WAL`): **Build phase** — create `trajectory_records_staging` and populate it with the retained post-compaction record set using bounded batches (default 1,000 records per `BEGIN IMMEDIATE` / `COMMIT`); **Swap phase** — a single brief `BEGIN IMMEDIATE` / COMMIT that renames `trajectory_records_staging` to `trajectory_records` (atomic SQLite rename). No direct `DELETE`-based compaction on `trajectory_records` is permitted — the staging-table swap is the required atomicity model. WAL mode ensures uncommitted frames after the last commit marker are discarded on next open; combined with the swap-phase `BEGIN IMMEDIATE` / `COMMIT` boundary, any mid-compact process crash at any of the four VP-019 crash points leaves the pre-compaction trajectory fully intact. Crash during the build phase leaves `trajectory_records_staging` partially filled; it MUST be dropped on recovery before the next compaction run (AC-020 / BC-2.04.011 {INV-003}).
...
- `pregolya-checkpoint/src/trajectory.rs` — `SqliteTrajectoryStore`, `TrajectoryCompactor` trait, DDL for `trajectory_records` table and `trajectory_records_staging` table (staging used during compaction swap phase), `put_record`/`replay`/`compact` implementations (compact uses two-phase staging-table swap per Architecture Rule 4)
...
| EC-009 | SIGKILL mid-compaction (any of the four VP-019 crash points) | Restart: `replay` returns pre-compaction records — staging-table swap model ensures pre-compaction `trajectory_records` is intact; uncommitted WAL frames discarded on next open; `trajectory_records_staging` dropped on recovery if present (BC-2.04.011 {INV-003}) |
