...
`put_record(record)` returns `Ok(())` when the record has been durably committed to the backing `checkpoint::trajectory` SQLite slice. Encryption is OPT-IN per BC-2.04.009 {INV-002}: `SqliteTrajectoryStore` accepts an `Option<Arc<dyn core::serializer::Serializer + Send + Sync>>` at construction (`core::serializer::Serializer` is the trait; `EncryptedSerializer` is the concrete implementor and MUST NOT be a direct dependency of `SqliteTrajectoryStore`). When a serializer is wired, `TrajectoryRecord::payload` is serialized via that serializer before the SQLite write; `event_kind` is persisted as cleartext because it is a discriminator field per BC-2.04.009 {PRE-002}. When no serializer is provided, payload is persisted as-is — there is NO fail-closed guard. Idempotency conflict detection (INV-001, see AC-007) compares PRE-SERIALIZATION PLAINTEXT payloads — NOT the stored ciphertext — to avoid falsely triggering E-TRAJ-002 under per-record nonce non-determinism. No `Ok(())` is returned unless the record is durably committed. Verified by `test_BC_2_04_009_put_record_returns_ok_on_commit()`.
...
### AC-018 (traces to BC-2.04.011 PC-004 — compaction is atomic; no partial state on storage error)
After any storage I/O error from `compact`, `replay(run_id)` returns the exact same result as before the attempted compaction. No partial compaction state is observable. Verified by `test_BC_2_04_011_compact_error_leaves_trajectory_intact()`.
...
- [ ] Implement `SqliteTrajectoryStore::compact` using WAL mode (`PRAGMA journal_mode=WAL`) and `BEGIN IMMEDIATE` / `DELETE` / `COMMIT` atomic transaction; storage I/O failure → `Err(PregolyaError { category: DURABILITY, code: "E-TRAJ-005" })` with full rollback (no partial compaction state)
...
4. `TrajectoryCompactor::compact` MUST enable WAL mode (`PRAGMA journal_mode=WAL`) on the SQLite connection and MUST use an atomic transaction (`BEGIN IMMEDIATE` / `COMMIT`). No non-transactional DELETE is permitted — WAL mode ensures uncommitted frames after the last commit marker are discarded on next open; combined with the `BEGIN IMMEDIATE` / `COMMIT` boundary, any mid-compact process crash leaves the trajectory fully intact (AC-020 / BC-2.04.011 {INV-003}).
...
| EC-009 | SIGKILL mid-compaction | Restart: `replay` returns pre-compaction records — uncommitted WAL frames discarded on next open (BC-2.04.011 {INV-003}) |
