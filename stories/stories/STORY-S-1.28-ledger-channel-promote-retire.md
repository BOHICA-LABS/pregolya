...
- [ ] In `channels/ledger.rs` — declare `#[non_exhaustive] pub struct LedgerChannel<T: LedgerEntry> { _inner: PhantomData<T> }` and provide the MANUAL bound-free `impl<T: LedgerEntry> Default for LedgerChannel<T> { fn default() -> Self { Self { _inner: PhantomData } } }` (canonical zero-sized marker shape per Rule 13; NOT `#[derive(Default)]` — derive emits spurious T: Default bound breaking Channel: Default; no Clone/Serialize/Deserialize; the Vec<T> accumulator lives outside the struct, managed by the BSP engine) and implement pure reducer `fn reduce(acc: Vec<T>, update: T) -> Vec<T>` (no Result; Vec linear scan per Rule 2 — no indexmap)
- [ ] In `channels/ledger.rs` — implement `graph::channels::Channel` for `LedgerChannel<T>` with `type Accumulator = Vec<T>` and `type Update = T` (BSP dispatch contract per Rule 15 / BC-2.02.007 {INV-004}); the MANUAL bound-free `impl<T: LedgerEntry> Default` (Rule 13) satisfies the `Channel: Default` supertrait without requiring `T: Default`
- [ ] Create `pregolya-graph/src/channels/promote_retire.rs` — declare `#[non_exhaustive] #[derive(Clone, Debug)] pub enum PromoteRetireOp<T: LedgerEntry> { Promote(T), Retire(String) }` (`Clone` mandatory per Rule 4 / `Channel::Update: Clone`; sound via `LedgerEntry: Clone`; `Debug` conventional; ADR-030 §Decision 3 round-58)
- [ ] In `channels/promote_retire.rs` — declare `#[non_exhaustive] pub struct PromoteRetireChannel<T: LedgerEntry> { _inner: PhantomData<T> }` and provide the MANUAL bound-free `impl<T: LedgerEntry> Default for PromoteRetireChannel<T> { fn default() -> Self { Self { _inner: PhantomData } } }` (canonical zero-sized marker shape per Rule 13; NOT `#[derive(Default)]` — derive emits spurious T: Default bound breaking Channel: Default; no Clone/Serialize/Deserialize; the active-set Vec<T> lives outside the struct, managed by the BSP engine) and implement pure reducer `fn reduce(acc: Vec<T>, update: PromoteRetireOp<T>) -> Vec<T>` (no Result; Vec linear scan for Promote-dedup per Rule 3 — no indexmap)
- [ ] In `channels/promote_retire.rs` — implement `graph::channels::Channel` for `PromoteRetireChannel<T>` with `type Accumulator = Vec<T>` and `type Update = PromoteRetireOp<T>` (BSP dispatch contract per Rule 15 / BC-2.02.009 {INV-004}); the MANUAL bound-free `impl<T: LedgerEntry> Default` (Rule 13) satisfies the `Channel: Default` supertrait without requiring `T: Default`
...
4. `PromoteRetireOp<T>` MUST carry `#[non_exhaustive]` AND `#[derive(Clone, Debug)]` (ADR-030 §Decision 3 round-58). Canonical form: `#[non_exhaustive] #[derive(Clone, Debug)] pub enum PromoteRetireOp<T: LedgerEntry> { Promote(T), Retire(String) }`. `Clone` is MANDATORY because the `Channel` trait requires `Update: Clone` — `PromoteRetireChannel<T>::Update = PromoteRetireOp<T>` — and the BSP engine must be able to clone update values. This derive is sound: `T: LedgerEntry` implies `T: Clone` via the supertrait bound, so no spurious bound is introduced and rustc #26925 is not triggered. `Debug` is conventional for a data-bearing enum (conditional on `T: Debug`; acceptable). This rule governs the data-bearing UPDATE ENUM only — it is DISTINCT from Rule 13's no-derives-on-marker-structs rule, which governs `LedgerChannel<T>` and `PromoteRetireChannel<T>` (zero-sized marker structs, not value types).
...
13. `LedgerChannel<T>` and `PromoteRetireChannel<T>` MUST have the canonical zero-sized marker shape with a MANUAL bound-free `Default` impl — `#[derive(Default)]` is FORBIDDEN on these types because derive emits a spurious `T: Default` bound that breaks the `Channel: Default` supertrait for callers with only `T: LedgerEntry` (ADR-030 §Decision 3 round-57). The canonical form is `#[non_exhaustive] pub struct LedgerChannel<T: LedgerEntry> { _inner: PhantomData<T> }` with `impl<T: LedgerEntry> Default for LedgerChannel<T> { fn default() -> Self { Self { _inner: PhantomData } } }` (and analogously `PromoteRetireChannel<T>`). No additional derives (`Clone`, `Serialize`, `Deserialize`, or any other) are permitted on the MARKER STRUCTS themselves — the marker type is a BSP dispatch tag, not a value type for serialization or cloning. This no-derives rule applies ONLY to the zero-sized MARKER STRUCTS (`LedgerChannel<T>` and `PromoteRetireChannel<T>`) — it does NOT govern `PromoteRetireOp<T>`, which is a data-bearing update enum that REQUIRES `#[derive(Clone, Debug)]` per Rule 4 (ADR-030 §Decision 3 round-58). The `Default` yield is `LedgerChannel { _inner: PhantomData }` / `PromoteRetireChannel { _inner: PhantomData }` (the zero-sized marker — NOT an empty `Vec<T>`; the accumulator `Vec<T>` is maintained externally by the BSP engine, not stored as a struct field). No `pub fn new()` constructor beyond `Default` is permitted. Use `LedgerChannel::default()` / `PromoteRetireChannel::default()` at call-sites. Cross-crate construction is blocked by both the private `_inner` field (primary: field privacy prevents struct-literal construction) and `#[non_exhaustive]` (secondary: confirms the annotation is present); both mechanisms are retained by design.
...
| 1.5 | 2026-09-01 | Round-54 fix-burst: F-P2A224-01b — Rule 15 provenance clause corrected per ADR-030 §Channel Trait Definition Home; S-1.28 consumes (does not define) the Channel trait; Channel impls for LedgerChannel<T> and PromoteRetireChannel<T> go in channels/ledger.rs and channels/promote_retire.rs respectively | story-writer |
