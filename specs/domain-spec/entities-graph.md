...
changelog:
  ...
  - "v1.12 (F-P171a-14/burst-273/2026-07-25): Fix HITL Approval Hook Domain intro — dependency-kind word corrected 'runtime' → 'compile-time' (corroborating carriers: ADR-018 §Decision 1 'cross-crate compile-time consumer', ADR-020 §Decision 1 'does NOT depend on pregolya-graph at compile time', dependency-graph.md crate-DAG annotation 'no pregolya-graph compile-time dep'). Date-monotonicity repair: v1.9 changelog date 2026-07-22 → 2026-07-23 (burst-242; corroborating carrier: api-surface.md changelog entry burst-242/2026-07-23). TD-VSDD-060 temporal-neighbor sweep: no additional inversions found in this file."
  - "v1.11 (F-P170-16/burst-272/2026-07-25): Fix HITL Approval Hook Domain intro — retire stale ActionRisk location claim 'pregolya-graph::hitl alongside ActionRisk'. ActionRisk relocated to pregolya-core (core::action_risk); pregolya-graph::hitl re-exports it. HITL hook entities and RiskGatePolicy remain in pregolya-graph::hitl. Placement rationale narrowed to hook types only. TD-VSDD-060 sweep: sole ActionRisk location claim in this file."
  ...
  - "v1.6 (2026-07-22): D23 entity additions (burst-230) — new section '## HITL Approval Hook Domain': PreToolCallHook, PreToolDecision, ToolCallPreview, ToolApprovalRequest (pregolya-graph::hitl, ADR-018). New section '## Context Compaction Domain': CompactionTrigger, CompactionPolicy, ConversationSnapshot, CompactionSummary (pregolya-core::budget + graph::budget, ADR-019). Tool entity updated: first-party subtypes from SS-23 added (ReadFileTool, WriteFileTool, EditFileTool, ListDirTool, BashTool, GrepTool). Relationships Summary extended. D23 added to decisions list."
  - "v1.5 (2026-07-21): F-P131-05 adjudication (burst-226) — PromptValue entity: MessageProvenance field renamed tag: Option<ProvenanceTag> → highest_trust_level: Option<TrustLevel>; Invariant updated from '.tag is Untrusted' to '.highest_trust_level is Some(TrustLevel::Untrusted)'. TrustLevel entity added to Retrieval and Serialization Domain (pregolya-prompts: prompts::template; 3 variants: Untrusted | UserInput | Trusted; severity ordering Untrusted > UserInput > Trusted; disambiguation from ProvenanceTag; authority ADR-015 §Decision 3 + CAP-022/023). Relationships Summary updated: ProvenanceTag line → TrustLevel. TD-VSDD-060 sibling sweep: no other ProvenanceTag trust-variant residue in this file."
  - "v1.4 (2026-07-20): D21 second-half entity additions — VectorStore (pregolya-vectorstores, ADR-014), Embeddings (pregolya-core core::embeddings, ADR-017), MetadataFilter (pregolya-vectorstores, ADR-014), SearchType (pregolya-vectorstores, ADR-014). Added to '## Retrieval and Serialization Domain'. Relationships Summary extended."
  - "v1.3 (2026-07-20): D21 entity additions — Document (pregolya-core core::documents, ADR-014), PromptValue (pregolya-prompts, ADR-015), Serialized (pregolya-core core::serializable, ADR-016). New section '## Retrieval and Serialization Domain' added. Relationships Summary extended. D21 added to decisions list."
...
- **Crate:** pregolya-core, module `core::documents`; no I/O. Pure Core classification.
...
- **Note:** Corresponds to LangChain's `Document { page_content, metadata }` type in
  langchain_core.documents.base (semport Corpus 1). `id` field is pregolya's addition for
...
- **Crate:** pregolya-prompts, module `prompts::prompt_value`
...
- **Note:** Corresponds to LangChain's `PromptValue` type; pregolya adds per-message
...
- **Crate:** pregolya-prompts, module `prompts::template`
...
- **Crate:** pregolya-core, module `core::serializable`
...
- **Note:** Corresponds to LangChain's `Serialized` TypedDict family in langchain_core.load.
  pregolya's `Serialized` is a proper Rust enum (serde `#[serde(tag="type")]`) rather than
...
- **Crate:** pregolya-vectorstores, module `vectorstores::store`
...
- **Note:** Corresponds to `VectorStore` abstract base class in LangChain v1
  (`langchain_core.vectorstores.base`). pregolya separates factory methods onto
...
backends and pregolya-memory semantic search (ADR-017 Decision 1).
...
- **Crate:** pregolya-core, module `core::embeddings`
- **Relationships:** InMemoryVectorStore holds Arc<dyn Embeddings>. pregolya-memory may
  hold Arc<dyn Embeddings> for semantic search. EmbeddingsOpenAI and EmbeddingsOllama implement
  Embeddings (first-party v1 impls). pregolya-anthropic has NO Embeddings impl (v1).
- **Note:** Corresponds to `Embeddings` abstract base class in LangChain v1
  (`langchain_core.embeddings.base`). pregolya uses `Vec<f32>` not ndarray (semport §8).
...
- **Crate:** pregolya-vectorstores, module `vectorstores::filter`
...
- **Note:** No direct LangChain v1 equivalent as a named type (filtering is handled
  per-backend in Python). pregolya promotes it to a named first-class domain type.
...
- **Crate:** pregolya-vectorstores, module `vectorstores::retriever`
...
- **Note:** Corresponds to `SearchType` enum in LangChain v1
  (`langchain_core.vectorstores.base`); pregolya's enum is identical in semantics.
...
> `pregolya-graph::hitl`. Placement rationale: no dependency-inversion need — no crate
> requires these hook types without depending on pregolya-graph (ADR-018 Decision 1).
> `ActionRisk` lives in `pregolya-core` (`core::action_risk`); `pregolya-graph::hitl`
> re-exports it (pregolya-tools needs `ActionRisk` without a pregolya-graph compile-time dep).
...
- **Crate:** pregolya-graph, module `graph::hitl`
...
- **Variants (`#[non_exhaustive]`):**
  ...
  - `PendingHumanApproval { prompt: Option<String> }` — suspend via `interrupt()` (BC-2.05.001 machinery reused); on `Command(resume=PreToolDecision)` the decision is applied; hook is NOT re-called on the resumed dispatch ("skip-hook-on-resume" invariant — BC-2.05.008)
- **Crate:** pregolya-graph, module `graph::hitl`
...
- **Crate:** pregolya-core, module `core::budget`; field of `BudgetConfig`
...
- **Interface:** `async fn compact(&self, snapshot: &ConversationSnapshot, run_ctx: &RunContext) → Result<CompactionSummary, PregolyaError>`
...
- **Crate:** pregolya-core, module `core::budget`
...
EmbeddingsOpenAI + EmbeddingsOllama implement Embeddings; pregolya-anthropic has NO impl
