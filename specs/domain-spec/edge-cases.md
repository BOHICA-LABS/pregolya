...
decisions: [D17]
changelog:
  - "v1.4 (NOTATION-SWEEP-DOMAIN-SPEC/2026-07-29): Error-construction notation corrections per ADR-010 §Error-Construction Notation Canon. 3 CLASS3_MISSING_DOTDOT violations corrected: added `, ..` before `}` in PregolyaError { category: TOOL, source: ToolException } (DEC-012), Err(PregolyaError { category: TRANSPORT }) (DEC-013 TCP-reset branch), and Err(PregolyaError { category: TIMEOUT }) (DEC-013 stall-timeout branch); all three were partial-field observations with no elision marker."
...
`PregolyaError { category: TOOL, source: ToolException, .. }`, not a generic opaque error.
...
`Err(PregolyaError { category: TRANSPORT, .. })`; accumulated partial tokens are discarded;
the error propagates to the caller.
**Expected behavior (per-chunk stall timeout):** The stream yields
`Err(PregolyaError { category: TIMEOUT, .. })`; accumulated partial tokens are discarded;
the error propagates to the caller.
