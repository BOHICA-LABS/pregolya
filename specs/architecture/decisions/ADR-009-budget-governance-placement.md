...
title: "Budget Governance Engine Placement (D17-Q4): pregolya-graph vs pregolya-core"
status: accepted
date: 2026-07-14
producer: architect
...
pregolya-core (pure primitives) or pregolya-graph (graph execution concern).

Three options:
1. **pregolya-core** — Budget as a standalone primitive; any Runnable can use it.
2. **pregolya-graph** — Budget as a graph-execution concern; evaluated per-LLM-call and per-tool-invocation within `tick()` (Collecting phase).
3. **pregolya-graph with BudgetPolicy trait in pregolya-core** — Trait in core, engine in graph.

## Decision: BudgetPolicy Trait in pregolya-core; Engine in pregolya-graph

**Chosen:** Option 3 — split at the trait boundary.

`BudgetPolicy` is a public trait in `pregolya-core`:
```rust
...
`BudgetEngine` (evaluation + EvidenceJournal) lives in `pregolya-graph::budget`:
...
- `EvidenceJournal` write is effectful → belongs in pregolya-graph with the scheduler.
- Placing the trait in pregolya-core allows non-graph `Runnable` implementations to use budget policies without a pregolya-graph dep.
- Budget escalation (BC-2.10.004) triggers a HITL interrupt, which requires pregolya-graph's interrupt queue — so the engine must be in pregolya-graph anyway.
...
- `pregolya-core` adds: `BudgetPolicy` trait, `RunContext`, `TokenUsage`, `PolicyDecision` types.
- `pregolya-graph` adds: `BudgetEngine`, `EvidenceJournal`.
...
- **Option 1 — pregolya-core only:** Budget as a standalone primitive any Runnable can use. Rejected because budget escalation (BC-2.10.004) requires the HITL interrupt queue which is a pregolya-graph concern; placing the engine in core would create an upward dependency.
- **Option 2 — pregolya-graph only:** Budget as a graph-execution concern with both trait and engine in pregolya-graph. Rejected because non-graph Runnables could not implement `BudgetPolicy` without depending on the full graph crate; trait belongs in core per ADR-009 Option 3 split.
- **Option 3 — split (chosen):** Trait in pregolya-core; engine in pregolya-graph. Satisfies purity boundary (trait is pure-core), crate layering (no upward dep), and HITL integration requirements.
