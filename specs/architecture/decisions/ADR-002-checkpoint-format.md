...
msgpack is the checkpoint wire format for `pregolya-checkpoint`. All `GraphState` and `CheckpointMetadata` serialization uses `rmp-serde`.

## Rationale

- Compact binary format (2-4× smaller than JSON for typical GraphState); important for soak-test durability and large checkpoints (Domain B multi-day runs).
...
| bincode 2.0.1 | Faster than msgpack but not self-describing; schema evolution (adding fields) requires explicit version tagging because bincode does not encode field names. Both bincode 1.x (legacy) and bincode 2.0.1 (current stable, breaking rewrite of 1.x) share this limitation. REJECT. |
| postcard 1.1.3 | Designed for embedded/`no_std` use cases; compact but also not self-describing (encodes as positional fields). The same schema-evolution concern applies as with bincode. REJECT. |
