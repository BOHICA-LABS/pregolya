...
superseded_by: null
changelog:
  - "1.11 (round-41/F-P2A172-02/2026-08-29): F-P2A172-02 [MED] DynRunnableAdapter<I,O,R> absent from §Exempt Structs. `DynRunnableAdapter<I,O,R>` is the concrete return type of `IntoDynRunnable::into_dyn()` (public API surface, declared `pub struct` in interface-definitions.md §DynRunnable and RunnableSequence). Criterion B: both fields are `pub(crate)` — `inner: R` (set at r39) and `_phantom: PhantomData<fn(I)->O>` (corrected r41/F-P2A172-03; interface-definitions.md §DynRunnable and RunnableSequence). External crates cannot construct `DynRunnableAdapter` regardless of `#[non_exhaustive]`; callers receive it only as the return value of `into_dyn()`. Adding `#[non_exhaustive]` provides no additional protection beyond the `pub(crate)`-field seal. Added row to §Exempt Structs; §Consequences and §Rationale updated: two→three exempt structs."
...
| `DynRunnableAdapter<I, O, R>` | pregolya-core (core::runnable) | B | Adapter struct wrapping any `R: Runnable<I,O>` for type-erased `DynRunnable` dispatch. Sealed from external construction by `pub(crate)` fields — `inner: R` and `_phantom: PhantomData<fn(I)->O>` are both `pub(crate)` (visibility set r39 and r41/F-P2A172-03 respectively; see interface-definitions.md §DynRunnable and RunnableSequence). External crates cannot construct it regardless of `#[non_exhaustive]`; callers receive it only as the return value of `IntoDynRunnable::into_dyn()`. Adding `#[non_exhaustive]` provides no additional protection beyond the `pub(crate)`-field seal. (round-41, F-P2A172-02) |
...
The Exempt Inventory (Decision 3) is small and principled: all nine exempt enums are closed by an external protocol contract or represent a fundamental operation set, where exhaustive matching is the correct behavior for downstream correctness. The three exempt structs are sealed from external construction regardless of `#[non_exhaustive]`: `GuardedDocuments` by a fully private inner field; `RunnableSequence` by `pub(crate)` fields (invisible outside the crate — round-40/F-P2A168-04); `DynRunnableAdapter` by `pub(crate)` fields on both `inner` and `_phantom` (round-41/F-P2A172-02+F-P2A172-03). In all three cases `#[non_exhaustive]` would add no additional protection beyond the field seal.
...
- The nine exempt enums (StreamEvent, MemoryScope, WriteGuardDecision, MemoryWriteRequest, SlotTrustPolicy, BoundaryType, IngressBoundary, GuardrailDecisionKind, GuardrailSeverityWire) and three exempt structs (GuardedDocuments, RunnableSequence, DynRunnableAdapter) correctly do NOT carry `#[non_exhaustive]`. Their absence is documented and justified in the Exempt Inventory.
...
- Product-owner action completed: `#[non_exhaustive]` applied to `ToolCallPreview` in interface-definitions.md §PreToolCallHook (burst-288); D-02 CLOSED (D-02 routing note in Required Inventory struct table).
