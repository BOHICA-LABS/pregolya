...
version: "1.22"
changelog:
  - "1.22 (round-49/F-P2A202-01/2026-08-30): F-P2A202-01 [OBS] — §BaseChatModel adjudication + §Send-Bounded RPITIT table updated for `bind_tools`/`with_structured_output` edition-2024 RPITIT capture. In Rust edition 2024, `impl Trait` in RPITIT return position auto-captures `&self` lifetime, making the return non-`'static`. `bind_tools(&self) -> Result<impl BaseChatModel, PregolyaError>` and `with_structured_output<T>(&self) -> impl Runnable<Vec<Message>, T>` are both affected. Decision (owned/escapable): both return `Box<dyn ...>` — `'static` by default; enables pipeline composition via `.pipe()`, `Arc` storage, `JoinSet::spawn`. `T` bound on `with_structured_output` gains `+ Send + 'static` for `Box<dyn Runnable + Send + Sync>` realizability. Primary fix in interface-definitions.md §BaseChatModel (v3.02)."
  - "1.21 (R44/F-P2A184-01+F-P2A184-03/2026-08-30): F-P2A184-01 [HIGH] §BaseChatModel adjudication — E0562 axis closed. `stream_chat` was declared `async fn` which desugars to nested `impl Trait` inside `impl Future<Output = Result<impl Stream<...>, PregolyaError>>` — not permitted on stable Rust (E0562 class; same as Runnable::stream R43/F-P2A180-01). Boxed fix applied in interface-definitions.md §BaseChatModel (R44): `fn stream_chat(...) -> impl std::future::Future<Output = Result<Pin<Box<dyn Stream<Item = Result<AiMessageChunk, PregolyaError>> + Send>>, PregolyaError>> + Send`. §BaseChatModel adjudication paragraph updated: 'stream_chat() returns `impl Stream`' replaced with 'stream_chat() is `async fn` (RPITIT `impl Future` with boxed `Pin<Box<dyn Stream...>>` output — E0562 fix R44)'; 'No changes to interface-definitions.md §BaseChatModel are required' qualified with dyn-NOT-required conclusion preserved + E0562 fix noted. §Send-Bounded RPITIT table BaseChatModel row updated: 'Not required; no fix applied' amended with E0562 axis record and boxed-fix disposition. F-P2A184-03 [MED] stale 'impl Stream return' prose updated: §Runnable adjudication 'impl Stream opaque returns (stream())' → 'RPITIT `impl Future` return (stream())'; §Tool adjudication 'returns `impl Stream`, non-dyn-compatible' → 'RPITIT `impl Future` return — opaque, non-dyn-compatible'. The dyn-NOT-required conclusions for both traits are unchanged."
  - "1.20 (R43/F-P2A180-01/2026-08-30): §Send-Bounded RPITIT inventory table Runnable row updated. `stream` method disposition column supplemented: `Runnable::stream` now returns `Pin<Box<dyn Stream<Item = Result<Output, PregolyaError>> + Send>>` (boxed to remove nested `impl Trait` inside `impl Future<Output = ..>`, which is not permitted on stable Rust — E0562 class; R43/F-P2A180-01). The `+ Send` RPITIT status (R34 fix) is unchanged; the boxed-stream form is an additional realizability fix applied in interface-definitions.md §Runnable. Has-+Send-RPITIT column entry updated to 'YES (after R34 fix); `stream` additionally uses `Pin<Box<dyn Stream<..> + Send>>` boxed return (E0562 closure, R43/F-P2A180-01)'. Primary fix in interface-definitions.md §Runnable."
  - "1.19 (R42/F-P2A176-01/2026-08-29): §Send-Bounded RPITIT — Corollary (`'static` vs `Send`) added after inventory table. A `+ Send` RPITIT future that borrows `&self` is NOT `'static`; default trait methods CANNOT call `JoinSet::spawn` on it (requires `F: Future + Send + 'static`). Canonical default-method implementation for `Runnable::batch` MUST use in-task `join_all` / `FuturesOrdered` — no spawn, order-preserving. An override holding `'static` / `Arc<Self>` state MAY use `JoinSet`. Mirrored in interface-definitions.md §Runnable batch doc-comment in same burst."
  - "1.18 (R39/F-P2A164-01/2026-08-29): §Send-Bounded RPITIT body updated to replace E0207-non-realizable R38 serde-bounded blanket with adapter model. The R38 form `impl<I, O, T> DynRunnable for T where T: Runnable<I, O> + ...` had `I` and `O` constrained only in the where-clause, not in Self type `T` (RFC 447 → E0207). Replaced with: adapter-wrapper model where `DynRunnableAdapter<I, O, R>` places all three params on Self (`R` directly; `I` and `O` via `PhantomData<fn(I) -> O>`), and `impl<I, O, R> DynRunnable for DynRunnableAdapter<I, O, R>` carries the serde bounds. §Send-Bounded RPITIT opening paragraph and §Note updated accordingly; RPITIT inventory table Runnable row unchanged (FIXED status holds). interface-definitions.md §DynRunnable adapter model is the authoritative source."
  - "1.17 (R38/F-P2A160-01/2026-08-29): §Send-Bounded RPITIT body updated to reflect serde-bounded blanket domain. The `impl<T: Runnable<..> + Send + Sync + 'static> DynRunnable for T` form in the opening paragraph and the concluding §Note updated to the R38 serde-bounded domain: `impl<I, O, T> DynRunnable for T where T: Runnable<I, O> + Send + Sync + 'static, I: serde::de::DeserializeOwned + Send + 'static, O: serde::Serialize + Send + 'static`. RPITIT inventory table Runnable row unchanged — +Send RPITIT remains FIXED in interface-definitions.md §Runnable; the blanket domain fix (serde bounds) is in interface-definitions.md §DynRunnable and RunnableSequence."
  - "1.16 (R34/F-P2A144-01/2026-08-29): §Adjacent Trait Object-Safety Adjudications — added §Send-Bounded RPITIT subsection. Records the canon rule: any native-async trait method whose future must be Send (boxed in #[async_trait] blanket impl, spawned on JoinSet, or stored behind Arc<dyn>) MUST use explicit RPITIT `+ Send` (not bare `async fn`), which is the stable-Rust mechanism to impose a Send bound on an RPITIT future without nightly Return-Type-Notation. Added exhaustive inventory table for round-34 sweep: Runnable (FIXED — invoke/stream/batch now carry +Send RPITIT in interface-definitions.md), Tool (inherits Runnable fix; no own async methods), BaseChatModel (zero dyn BaseChatModel sites — stream_chat future need not be Send; no fix required), SkillStore (zero dyn SkillStore sites — confirmed in round-33 object-safety audit; no fix required). Updated existing Runnable adjudication paragraph to note that the dyn-NOT-required conclusion is unchanged; the Send-RPITIT change addresses blanket-impl composition realizability, not dyn-compat of Runnable itself."
...
`Runnable<Input, Output>` is a generic trait with RPITIT opaque returns: `stream()` returns an `impl Future` (outer RPITIT `+ Send`) whose `Output` is `Result<Pin<Box<dyn Stream<...> + Send>>, PregolyaError>` (stream boxed since R43/F-P2A180-01 to remove nested `impl Trait` — E0562 constraint); `pipe()` is bounded by `where Self: Sized` (explicitly excludes `dyn T`). These characteristics make `dyn Runnable<Input, Output>` non-trivially dyn-compatible. However, the spec corpus contains **zero** instances of `dyn Runnable<...>`. The architecture resolves heterogeneous pipeline composition through a separate `Arc<dyn DynRunnable>` type-erased composition path (BC-2.01.003/BC-2.01.004; interface-definitions.md §DynRunnable and RunnableSequence, E-CORE-004 note). `Runnable<Input, Output>` is always monomorphized — the `pipe()` combinator and concrete stage types are statically dispatched. The `impl Stream` return in `Runnable::stream()` and the `impl Runnable` return in `pipe()` are therefore **non-issues** for the production architecture. *(R34 addendum: `Runnable`'s async methods were changed to explicit RPITIT `+ Send` to make the blanket `DynRunnable`/`DynTool` impls realizable on stable Rust — this does NOT change the dyn-NOT-required conclusion; see §Send-Bounded RPITIT below.)*

**Authority:** semport/core/rust-translation-strategy.md §Cross-cutting open design questions (candidate ADRs) (recommends separate `DynRunnable` as candidate ADR); BC-2.01.003 EC-001 and BC-2.01.004 EC-001 and TV-004 (spec `Arc<dyn DynRunnable>` as the type-erased composition path).

**`BaseChatModel` — axis settled, dyn NOT required:**

`BaseChatModel: Runnable<Vec<Message>, AiMessage>` inherits all of `Runnable`'s non-dyn characteristics and adds its own: `stream_chat()` is declared `async fn` (RPITIT `impl Future` with boxed `Pin<Box<dyn Stream<Item = Result<AiMessageChunk, PregolyaError>> + Send>>` output — E0562 fix R44/F-P2A184-01; see interface-definitions.md §BaseChatModel), `bind_tools()` returns `Box<dyn BaseChatModel + Send + Sync>`, and `with_structured_output<T: ... + Send + 'static>()` returns `Box<dyn Runnable<Vec<Message>, T> + Send + Sync>`. The spec corpus contains **zero** instances of `dyn BaseChatModel` or `Arc<dyn BaseChatModel>`. Provider crates exclusively use static dispatch: `impl BaseChatModel for ChatOpenAI`, `impl BaseChatModel for ChatAnthropic`, `impl BaseChatModel for ChatOllama` (system-overview.md §Crate Topology, module-decomposition.md §Provider Crates and Standard Tests). The dyn-NOT-required conclusion is unchanged. *(R44 addendum: `stream_chat`'s `async fn` form desugared to nested `impl Trait` inside `impl Future<Output = Result<..>>` — E0562 on stable Rust (same class as Runnable::stream R43). Boxed fix applied in interface-definitions.md §BaseChatModel. The fix addresses the static-dispatch monomorphized path; it does NOT change the dyn-NOT-required conclusion — zero `dyn BaseChatModel` sites exist in the corpus.)* *(Round-49/F-P2A202-01 addendum: `bind_tools(&self) -> impl BaseChatModel` and `with_structured_output<T>(&self) -> impl Runnable<Vec<Message>, T>` in Rust edition 2024 auto-capture `&self`, making RPITIT returns non-`'static` and preventing pipeline composition, storage, and spawning. Decision: owned/escapable `Box<dyn ...>` form. See §Send-Bounded RPITIT table BaseChatModel row.)*

**`Tool` — axis settled, DynTool required (option b):**

`Tool: Runnable<ToolInput, ToolOutput>` inherits `Runnable`'s `stream()` method (RPITIT `impl Future` return — opaque, non-dyn-compatible; stream boxed to `Pin<Box<dyn Stream...>>` inside the Future output since R43/F-P2A180-01 E0562 fix) and `pipe()` method (`impl Runnable` + `where Self: Sized` bound). These inherited characteristics make `dyn Tool` non-trivially NOT object-safe (dyn-incompatible under E0038); any caller that attempts direct vtable dispatch via `Arc<dyn Tool>` will fail to compile. Corpus re-verification (FIX-BURST-277-WAVE-B errata) found **2 live `Arc<dyn pregolya_core::Tool>` usage sites** in MCP BCs (BC-2.09.001 and BC-2.09.002).

Resolution: **option (b)** — mint `DynTool` as a type-erased object-safe seam mirroring the `Arc<dyn DynRunnable>` pattern (BC-2.01.003/BC-2.01.004). `DynTool` exposes `async fn invoke_dyn(&self, input: serde_json::Value) -> Result<serde_json::Value, PregolyaError>` plus the `name()`, `description()`, `schema()`, and `action_risk()` methods from `Tool`. It is a separate, fully object-safe trait; `Arc<dyn DynTool>` compiles without E0038.
...
**Send-Bounded RPITIT for Native-Async Traits Bridged to `#[async_trait]` Façades — Canon Rule (R34/F-P2A144-01):**

The original `Runnable<Input, Output>` adjudication above addressed dyn-compatibility of `Runnable<Input, Output>` itself; it concluded dyn was not required. This supplemental record addresses a distinct property: blanket-impl realizability for `DynRunnable` and `DynTool` on stable Rust.

When the `#[async_trait]` macro generates the adapter-wrapper `impl<I, O, R> DynRunnable for DynRunnableAdapter<I, O, R> where R: Runnable<I, O> + Send + Sync + 'static, I: serde::de::DeserializeOwned + Send + 'static, O: serde::Serialize + Send + 'static` (R39/F-P2A164-01; interface-definitions.md §DynRunnable and RunnableSequence), it desugars `DynRunnable::invoke` into a method returning `Pin<Box<dyn Future<Output = ..> + Send>>`. The body of that method boxes the future returned by `R::invoke()`. For the `Send` box cast to compile on stable Rust, the future from `R::invoke()` must be provably `Send`. On stable Rust, bare `async fn invoke(..)` in a trait definition carries no `Send` bound on its RPITIT future. Return-Type-Notation (RTN), which would allow `fn invoke(..) -> impl Future + Send where R::invoke(): Send`, requires `#![feature(return_type_notation)]` (nightly-only). The only stable mechanism is explicit RPITIT: `fn invoke(..) -> impl Future<Output = ..> + Send`.

The R38 form `impl<I, O, T> DynRunnable for T where T: Runnable<I, O> + ...` was non-realizable: `I` and `O` appeared only in the where-clause, not in the Self type `T` (RFC 447 → E0207). The adapter-wrapper resolves E0207 by placing all three type params on `DynRunnableAdapter<I, O, R>` — `R` directly; `I` and `O` via `PhantomData<fn(I) -> O>` in the struct fields. Each (I, O, R) triple yields a distinct Self type → no E0119 coherence overlap.

**Canon rule:** Any native-async trait method (defined without `#[async_trait]`) whose future must be `Send` — because it is (a) boxed inside a `#[async_trait]` blanket impl, or (b) spawned on `tokio::task::JoinSet`, or (c) stored behind `Arc<dyn>` — MUST be declared with explicit RPITIT `+ Send` rather than bare `async fn`. This rule is POL-18-enforceable at Phase 3 compile time; the compiler diagnostic is E0277.

**Exhaustive inventory as of R34:**

| Trait | Native-async methods | Bridge requiring Send future | Has +Send RPITIT? | Disposition |
|-------|---------------------|------------------------------|-------------------|-------------|
| `Runnable<Input, Output>` | `invoke`, `stream`, `batch` | Blanket → `DynRunnable`; via `Tool: Runnable<ToolInput, ToolOutput>`, blanket → `DynTool` | YES (after R34 fix); `stream` additionally uses `Pin<Box<dyn Stream<Item = Result<Output, PregolyaError>> + Send>>` boxed return to remove nested `impl Trait` inside `impl Future<Output = ..>` (E0562 closure, R43/F-P2A180-01) | FIXED in interface-definitions.md §Runnable |
| `Tool` | None (inherits from `Runnable`; no own async methods) | Blanket → `DynTool` | N/A — inherited via Runnable | Inherits Runnable fix |
| `BaseChatModel` | `stream_chat` (own) + inherits `invoke`/`stream`/`batch` + `bind_tools`/`with_structured_output` (own, non-async) | None — zero `dyn BaseChatModel` sites; static dispatch only | N/A — not behind dyn | E0562 axis (R44/F-P2A184-01): `stream_chat` was `async fn` desugaring to nested `impl Trait` inside `impl Future<Output = Result<impl Stream<...>, ..>>` — E0562 on stable Rust. Boxed fix in interface-definitions.md §BaseChatModel: `fn stream_chat(...) -> impl Future<Output = Result<Pin<Box<dyn Stream<Item = Result<AiMessageChunk, PregolyaError>> + Send>>, PregolyaError>> + Send`. `+ Send` on the RPITIT future is NOT required (zero dyn sites); box is required for E0562. RPITIT capture axis (round-49/F-P2A202-01): `bind_tools(&self) -> impl BaseChatModel` and `with_structured_output<T>(&self) -> impl Runnable<Vec<Message>, T>` auto-capture `&self` in edition 2024, making the returns non-`'static`. Decision: `Box<dyn BaseChatModel + Send + Sync>` and `Box<dyn Runnable<Vec<Message>, T> + Send + Sync>` respectively; `T` gains `+ Send + 'static`. Owned/escapable; enables pipeline composition, Arc storage, spawning. |
| `SkillStore` | `load_skill`, `list_skills`, `skill_exists` | None — zero `dyn SkillStore` sites confirmed in round-33 object-safety audit | N/A — not behind dyn | Not required; no fix applied |

**Corollary — `'static` vs `Send` (R42/F-P2A176-01):** a `+ Send` RPITIT future that borrows `&self` is NOT `'static`. `JoinSet::spawn` requires `F: Future + Send + 'static`; spawning in a default `&self` trait method is IMPOSSIBLE because the future would borrow the `&self` receiver that goes out of scope at the spawn site. The canonical default-method implementation for `Runnable::batch` MUST use in-task cooperative concurrency — `futures::future::join_all` or `FuturesOrdered` — polling all `invoke` futures within the calling task without spawning (no `'static` requirement; order preserved per {INV-002}). An implementor that holds `'static` / `Arc<Self>` state MAY override `batch` with a `JoinSet`-based implementation for true task-level parallelism. This corollary is reflected in the `interface-definitions.md §Runnable batch` doc-comment.

**Note on `Runnable<Input, Output>` adjudication above:** the dyn-NOT-required conclusion from burst 119 (Runnable is always monomorphized; the `Arc<dyn DynRunnable>` path is the separate type-erased seam) is unchanged by the R34 RPITIT fix, the R38 serde-bounds domain fix, and the R39 adapter-model fix. Neither change introduces any `dyn Runnable<..>` usage sites; the adapter-wrapper `impl<I, O, R> DynRunnable for DynRunnableAdapter<I, O, R>` (R39/F-P2A164-01) is the realizable form that makes E-CORE-003 reachable on stable Rust. The R38 serde-bounded blanket `impl<I, O, T> DynRunnable for T where T: Runnable<I, O> + ...` was E0207-non-realizable and has been superseded.
