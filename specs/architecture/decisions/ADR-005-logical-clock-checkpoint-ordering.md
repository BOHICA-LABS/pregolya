...
| `BaseChatModel` | `stream_chat` (own) + inherits `invoke`/`stream`/`batch` | None — zero `dyn BaseChatModel` sites; static dispatch only | N/A — not behind dyn | E0562 axis (R44/F-P2A184-01): `stream_chat` was `async fn` desugaring to nested `impl Trait` inside `impl Future<Output = Result<impl Stream<...>, ..>>` — E0562 on stable Rust. Boxed fix in interface-definitions.md §BaseChatModel: `fn stream_chat(...) -> impl Future<Output = Result<Pin<Box<dyn Stream<Item = Result<AiMessageChunk, PregolyaError>> + Send>>, PregolyaError>> + Send`. `+ Send` on the RPITIT future is NOT required (zero dyn sites); box is required for E0562. `stream_chat` future need not be `Send` (Send-RPITIT unchanged). |
