...
**Note on `Runnable<Input, Output>` adjudication above:** the dyn-NOT-required conclusion from burst 119 (Runnable is always monomorphized; the `Arc<dyn DynRunnable>` path is the separate type-erased seam) is unchanged by this R34 fix. The RPITIT `+Send` change does not introduce any `dyn Runnable<..>` usage sites; it only makes the blanket `impl<T: Runnable<..> + Send + Sync + 'static> DynRunnable for T` realizable on stable Rust.
