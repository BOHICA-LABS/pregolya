...
When the `#[async_trait]` macro generates the serde-bounded bridging blanket `impl<I, O, T> DynRunnable for T where T: Runnable<I, O> + Send + Sync + 'static, I: serde::de::DeserializeOwned + Send + 'static, O: serde::Serialize + Send + 'static` (R38/F-P2A160-01; interface-definitions.md §DynRunnable and RunnableSequence), it desugars `DynRunnable::invoke` into a method returning `Pin<Box<dyn Future<Output = ..> + Send>>`. The body of that method boxes the future returned by `T::invoke()`. For the `Send` box cast to compile on stable Rust, the future from `T::invoke()` must be provably `Send`. On stable Rust, bare `async fn invoke(..)` in a trait definition carries no `Send` bound on its RPITIT future. Return-Type-Notation (RTN), which would allow `fn invoke(..) -> impl Future + Send where T::invoke(): Send`, requires `#![feature(return_type_notation)]` (nightly-only). The only stable mechanism is explicit RPITIT: `fn invoke(..) -> impl Future<Output = ..> + Send`.
...
**Note on `Runnable<Input, Output>` adjudication above:** the dyn-NOT-required conclusion from burst 119 (Runnable is always monomorphized; the `Arc<dyn DynRunnable>` path is the separate type-erased seam) is unchanged by the R34 RPITIT fix and the R38 serde-bounds domain fix. Neither change introduces any `dyn Runnable<..>` usage sites; the serde-bounded bridging blanket `impl<I, O, T> DynRunnable for T where T: Runnable<I, O> + Send + Sync + 'static, I: serde::de::DeserializeOwned + Send + 'static, O: serde::Serialize + Send + 'static` makes the blanket realizable and E-CORE-003 reachable on stable Rust (R38/F-P2A160-01).
