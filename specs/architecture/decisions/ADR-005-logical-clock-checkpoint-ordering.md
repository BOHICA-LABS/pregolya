...
| `BaseChatModel` | `stream_chat` (own) + inherits `invoke`/`stream`/`batch` | None — zero `dyn BaseChatModel` sites; static dispatch only | N/A — not behind dyn | Not required; `stream_chat` future need not be `Send`; no fix applied |
