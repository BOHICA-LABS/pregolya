...
| `mcp::registry` | `ToolRegistry` — runtime tool registration store: `Arc<RwLock<HashMap<String, Arc<dyn DynTool>>>>`; `register(name, tool)` / `get(name)` / `list()` operations; shared by `mcp::server` (reads for tools/list + tools/call dispatch) and `mcp::client` (populates at session startup via mcp::discovery conversion); canonical file: `pregolya-mcp/src/registry.rs` (S-2.11 Task 4); BC-2.09.006/BC-2.09.007/BC-2.09.008 §Architecture Anchors | MEDIUM |
