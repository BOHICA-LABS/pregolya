...
| E-CORE-006 | INTERNAL | broken | BC-2.01.003 | `RecursionLimitExceeded: nested invoke/stream call depth <depth> exceeded recursion_limit <limit>` |
...
| E-PROV-008 | TRANSPORT | broken | BC-2.08.004 | `ProviderHttpError: provider '<provider>' returned HTTP <status>: <body_preview>` — **(ADV-P1D-PASS-56-COMPLETION):** Generic provider HTTP 5xx and unparseable-error-body responses. Covers two scenarios that share TRANSPORT category: (a) HTTP 5xx with non-JSON body (e.g., HTML error page); (b) HTTP 4xx/5xx where the response body does not match the provider's documented error schema. Both are categorized TRANSPORT because the caller cannot extract structured error information — the response is opaque from ferrochain's perspective. Semantically distinct from E-PROV-003 (StreamInterrupted, mid-stream TCP reset) and E-PROV-002 (ProviderTimeout, per-chunk stall). RetryHint: Later (TRANSPORT category default). Surfaced embedded in Run.error in v1; categorical TRANSPORT→502 fallback applies only if directly surfaced via HTTP. |
| E-PROV-009 | VAL | broken | BC-2.08.013 | `ToolCallDialectParseError: <dialect> failed to deserialize tool calls from content: <reason>` — **(D20 sub-burst 2):** Raised when `ToolCallDialect::deserialize_tool_calls` returns `Err`. The content block was received from the provider but does not conform to the dialect's expected format (e.g., malformed Hermes ChatML XML). VAL — the content is caller-supplied (provider-returned) data that violates the dialect contract. RetryHint: Never (VAL default — the same dialect+content pair will always fail). |
| E-PROV-010 | POLICY | broken | BC-2.08.014 | `ProviderChainExhausted: all <chain_length> providers in fallback chain failed — last error: <last_error>` — **(D20 sub-burst 2):** Raised when `ProviderFallbackPolicy` has attempted every provider in the ordered chain and all returned retryable errors (429/5xx/auth). POLICY — the policy constraint (exhausted chain) rejects the operation; the caller must reconfigure the chain or fix the underlying provider issues. RetryHint: Never (POLICY default — adding more providers requires config change, not retry). |
