...
> **Column guide:** Module = canonical `crate::module` path (or crate name for crate-level rows). Qualifier = sub-component discriminator or crate-level annotation. Crate-level rows annotated `crate-level` (xtask, ferrochain-community, ferrochain-standard-tests, openai/anthropic/ollama BaseChatModel, ferrochain-macros) are excluded from exact 1:1 module census matching; the ferrochain-macros crate-level row is a roll-up — its census-authoritative entries are the individual macros::tool/entrypoint/task module rows. Multi-aspect modules (core::serializable) with two rows use (Module, Qualifier) as the composite key for per-row identification; the census treats the module as matched when any registry row covers the Module cell.

| Module | Qualifier | Crate | SS | Tier | VP | Kill Rate | Phase Gate |
|--------|-----------|-------|-----|------|-----|-----------|-----------|
| `graph::bsp_engine` | reducer stage | ferrochain-graph | SS-03 | CRITICAL | VP-001 | ≥ 95% | P3 per-story + P5 |
| `graph::scheduler` | — | ferrochain-graph | SS-03 | CRITICAL | — | ≥ 95% | P3 per-story + P5 |
| `graph::hitl` | — | ferrochain-graph | SS-05 | CRITICAL | — | ≥ 95% | P3 per-story + P5 |
| `checkpoint::session_index` | — | ferrochain-checkpoint | SS-04 | CRITICAL | VP-002 | ≥ 95% | P3 per-story + P5 |
| `checkpoint::clock` | — | ferrochain-checkpoint | SS-04 | CRITICAL | — | ≥ 95% | P3 per-story + P5 |
| `checkpoint::encryption` | — | ferrochain-checkpoint | SS-04 | CRITICAL | — | ≥ 95% | P3 per-story + P5 |
| `sandbox::path_guard` | — | ferrochain-sandbox | SS-13 | CRITICAL | VP-003 | ≥ 95% | P3 per-story + P5 |
| `core::credentials` | — | ferrochain-core | SS-14 | CRITICAL | — | ≥ 95% | P3 per-story + P5 |
| `core::error` | — | ferrochain-core | SS-14 | CRITICAL | — | ≥ 95% | P3 per-story + P5 |
| `graph::channels` | — | ferrochain-graph | SS-02 | HIGH | — | ≥ 90% | P5 |
| `graph::budget` | — | ferrochain-graph | SS-10 | HIGH | — | ≥ 90% | P5 |
| `graph::provenance` | — | ferrochain-graph | SS-11 | HIGH | — | ≥ 90% | P5 |
| `core::runnable` | — | ferrochain-core | SS-01 | HIGH | — | ≥ 90% | P5 |
| `core::message` | — | ferrochain-core | SS-01 | HIGH | — | ≥ 90% | P5 |
| `server::handlers` | — | ferrochain-server | SS-12 | HIGH | — | ≥ 90% | P5 |
| `server::security` | — | ferrochain-server | SS-12 | HIGH | — | ≥ 90% | P5 |
| `openai` | crate-level — BaseChatModel impl | ferrochain-openai | SS-08 | HIGH | — | ≥ 90% | P5 |
| `anthropic` | crate-level — BaseChatModel impl | ferrochain-anthropic | SS-08 | HIGH | — | ≥ 90% | P5 |
| `ollama` | crate-level — BaseChatModel impl | ferrochain-ollama | SS-08 | HIGH | — | ≥ 90% | P5 |
| `checkpoint::lineage` | — | ferrochain-checkpoint | SS-04 | HIGH | — | ≥ 90% | P5 |
| `ferrochain-macros` | crate-level — proc-macro crate | ferrochain-macros | — | HIGH | — | ≥ 90% | P5 |
| `macros::tool` | — | ferrochain-macros | SS-08 | HIGH | — | ≥ 90% | P5 |
| `macros::entrypoint` | — | ferrochain-macros | SS-08 | HIGH | — | ≥ 90% | P5 |
| `macros::task` | — | ferrochain-macros | SS-08 | HIGH | — | ≥ 90% | P5 |
| `memory::write_guard` | — | ferrochain-memory | SS-15 | HIGH | — | ≥ 90% | P5 |
| `checkpoint::sqlite` | — | ferrochain-checkpoint | SS-04 | MEDIUM | — | ≥ 80% | P5 |
| `splitters::recursive` | — | ferrochain-splitters | SS-07 | MEDIUM | — | ≥ 80% | P5 |
...
| `ferrochain-standard-tests` | crate-level | ferrochain-standard-tests | SS-08 | MEDIUM | — | ≥ 80% | P5 |
| `core::retry` | — | ferrochain-core | SS-16 | MEDIUM | — | ≥ 80% | P5 |
| `memory::store` | — | ferrochain-memory | SS-15 | MEDIUM | — | ≥ 80% | P5 |
| `graph::event_emitter` | — | ferrochain-graph | SS-06 | MEDIUM | — | ≥ 80% | P5 |
| `xtask` | crate-level | xtask | SS-17 | LOW | — | n/a (excluded from cargo-mutants) | advisory |
| `ferrochain-community` | crate-level | ferrochain-community | — | LOW | — | n/a (excluded from cargo-mutants) | advisory |
| `core::serializable` | Reviver — allowlist containment | ferrochain-core | SS-19 | CRITICAL | VP-010 | ≥ 95% | P3 per-story + P5 |
| `vectorstores::similarity` | — | ferrochain-vectorstores | SS-21 | CRITICAL | VP-009 | ≥ 95% | P3 per-story + P5 |
| `prompts::injection_guard` | — | ferrochain-prompts | SS-18 | HIGH | VP-006 | ≥ 90% | P3 per-story + P5 |
...
| `mcp::ingress` | — | ferrochain-mcp | SS-09 | HIGH | — | ≥ 90% | P5 |
...
| `vectorstores::store` | — | ferrochain-vectorstores | SS-21 | MEDIUM | — | ≥ 80% | P5 |
| `vectorstores::retriever` | — | ferrochain-vectorstores | SS-20 | MEDIUM | — | ≥ 80% | P5 |
| `vectorstores::memory` | — | ferrochain-vectorstores | SS-21 | MEDIUM | — | ≥ 80% | P5 |
| `openai::embeddings` | — | ferrochain-openai | SS-22 | HIGH | — | ≥ 90% | P5 |
| `ollama::embeddings` | — | ferrochain-ollama | SS-22 | HIGH | — | ≥ 90% | P5 |
...
> DECISION: `ferrochain-macros` receives a HIGH-tier crate-level row (annotated Qualifier column).
> Consistent with OBS-P31-1 (prd-supplements/module-criticality.md) and orchestrator adjudication
> in ADV-P1D-PASS-32. `ferrochain-community` retains a LOW row as a placeholder for post-v1
> third-party contributions; it is not in-tree at v1.
>
> **F-P172b-18 adjudication (v2.1):** `xtask` and `ferrochain-community` excluded from
...
> Module/crate breakdown: 12 CRITICAL module-level + 24 HIGH module-level (incl. macros::tool, macros::entrypoint, macros::task added FIX-BURST-275-reopened) + 4 HIGH crate-level (openai/anthropic/ollama BaseChatModel + ferrochain-macros crate-level roll-up) + 34 MEDIUM module-level + 1 MEDIUM crate-level (ferrochain-standard-tests) + 2 LOW crate-level (xtask + ferrochain-community) = 77 rows total.
...
>
> **Iron Law gap — eval::judge (v2.1):** `ferrochain-standard-tests::eval::judge` added as MEDIUM module row. The `eval.judge_infra_error` structured event in observability.md identifies this module as the emitter. BC-2.08.013/014 scope the LLM judge conformance behavior. The module executes LLM judge calls (async I/O, Effectful Shell), has behavioral contracts, and emits structured events — all three criteria that require an Iron Law entry in module-decomposition.md (see also purity-boundary-map.md Effectful Shell row added in same burst). The pre-existing `ferrochain-standard-tests` crate-level row remains as a crate-level annotation; `eval::judge` is the module-level row that satisfies Iron Law for the specific module with observable behavior.
