...
Every pregolya provider chat model must map provider-specific HTTP error responses to
typed `PregolyaError` variants with the correct `category` field — ensuring that
...
**Expected behavior:** `PregolyaError { category: AUTH, code: E-PROV-004, message: "ProviderAuthFailed: authentication failed", .. }`.
...
**Expected behavior:** `Err(PregolyaError { category: VAL, code: E-PROV-006,
...
**Expected behavior:** `Err(PregolyaError { category: RATE, code: E-PROV-001, retry_hint: RetryHint::Later(Duration::from_secs(60)),
...
**Expected behavior:** `Err(PregolyaError { category: TRANSPORT, code: E-PROV-008, message: "ProviderHttpError: provider
...
**Expected behavior:** `Err(PregolyaError { category: TRANSPORT, code: E-PROV-008, message: "ProviderHttpError: unknown error
...
|---|-------|-----------------|-------|
| TV-001 | Cassette: HTTP 401 | `Err(PregolyaError { category: AUTH, code: E-PROV-004, .. })` — key not in message | Auth error |
| TV-002 | Cassette: HTTP 400 "context length exceeded" | `Err(PregolyaError { category: VAL, code: E-PROV-006, .. })` | Context overflow (E-PROV-006 ContextLengthExceeded). PASS-ABBREV via EC-002. |
| TV-003 | Cassette: HTTP 429 with `Retry-After: 30` | `Err(PregolyaError { category: RATE, code: E-PROV-001, retry_hint: Later(30s), .. })` | Rate limit. PASS-ABBREV via EC-003. |
| TV-004 | Cassette: HTTP 500 | `Err(PregolyaError { category: TRANSPORT, code: E-PROV-008, .. })` | Provider 5xx — code deferred |
| TV-005 | Cassette: HTTP 400 unknown JSON body | `Err(PregolyaError { category: TRANSPORT, code: E-PROV-008, .. })` — no panic | Unknown format — code deferred |
...
- `pregolya-<provider>/src/error_mapping.rs` — provider HTTP status → PregolyaError (to be created)
- `pregolya-standard-tests/src/chat_models/error_fidelity.rs` — error fidelity battery (to be created)
...
| Capability Anchor Justification | CAP-009 ("Provider-Conformant Chat Model Interface") per capabilities-p1-p2.md §CAP-009 — this BC specifies the typed error mapping requirement for every provider implementation; CAP-011 ("Provider Conformance Suite (Standard Tests)") per capabilities-p1-p2.md §CAP-011 — this BC expresses the error-fidelity subset of pregolya-standard-tests |
