...
`Retriever` is a dyn-compatible async trait in `ferrochain-core: core::retriever` with a single
abstract method: `async fn get_relevant_documents(&self, query: &str) → Result<Vec<Document>, FerrochainError>`.
Graph nodes that perform RAG operations hold `Arc<dyn Retriever>` without depending on
`ferrochain-vectorstores`. The `Document` type (`ferrochain-core: core::documents`) is the
...
1. `ferrochain-core` Cargo.toml includes `async-trait` as a dependency.
...
2. `get_relevant_documents(&self, query: &str) → Result<Vec<Document>, FerrochainError>`:
...
4. The `Retriever` trait resides in `ferrochain-core: core::retriever`, NOT in
   `ferrochain-vectorstores`. Graph crates depend only on `ferrochain-core` for `Arc<dyn Retriever>`.
