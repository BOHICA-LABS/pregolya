...
changelog:
  ...
  - "1.9 (round-49/F-P2A207-03/2026-08-31): F-P2A207-03 [HIGH] — §Architecture Anchors phantom anchor corrected: `pregolya-core/src/context.rs` does not exist; canonical module is `core::invocation_context` at `pregolya-core/src/invocation_context.rs` (added by architect in interface-definitions.md §InvocationContext; module-decomposition.md §InvocationContext). File path and module path corrected."
traces_to:
...
1. {PRE-001} An MCP tool call has returned a `CallToolResult` with `isError = false`.
2. {PRE-002} An `InvocationContext` is associated with the current graph run.
3. {PRE-003} A `GuardrailHook` may or may not be registered in the `InvocationContext`.

## Postconditions

1. {PC-001} Before any converted content block enters the model context (i.e., before
...
2. {PC-002} If the hook accepts: the content blocks proceed normally into the model context.
3. {PC-003} If the hook rejects: the content blocks do NOT enter the model context. The
...
4. {PC-004} If no `GuardrailHook` is registered: the content blocks are permitted through
...
5. {PC-005} The guardrail fires unconditionally on every non-error MCP tool result;
   there is no opt-out code path for tool-result ingress (DI-012).

## Invariants

- {INV-001} DI-012: Guardrail hooks fire on tool-result ingress — not only on user-input and
  model-output boundaries. Content that fails a guardrail does not enter the model
  context under any code path.
- {INV-002} The guardrail hook fires AFTER content-block conversion (the blocks passed to
  the hook are pregolya `ContentBlock` values, not raw MCP wire bytes).
- {INV-003} The `ProvenanceTag` is always attached with `boundary_type: BoundaryType::ToolResult`; it is not optional.
- {INV-004} The hook is synchronous-or-async consistent: async hooks are awaited in the
...
**Expected behavior:** The rejection `ToolMessage` contains a pregolya-generated
fallback block:
...
- `pregolya-core/src/invocation_context.rs` (`core::invocation_context`) — `InvocationContext` (hook registry; canonical module per interface-definitions.md §InvocationContext v3.02)
...
| Module | pregolya-mcp / pregolya-core (guardrail hook traits) |
