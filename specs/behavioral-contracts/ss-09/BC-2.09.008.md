...
4. {PC-004} On successful graph execution: `GraphRunner::run` runs the graph to a terminal
   ...
   The server serializes this per BC-2.09.007 {PC-002} (per the BC-2.09.007 {PC-002}
   result_text selection rule: `Value::String(s)` → `s` verbatim; all other `Value`
   variants → `serde_json::to_string(&extract_output_result)`). If `extract_output` returns
...
`PendingHumanApproval` for a tool with `action_risk = Some(ActionRisk::ReadOnly)` (gate
approves, `ReadOnly < Medium`); later in the same run a node calls `interrupt()`.
**Expected behavior:** The `ActionRisk` gate approves (`ReadOnly < Medium`) — the inner
`PreToolCallHook` is invoked; `PendingHumanApproval` is overridden to `Approve` — the tool
...
**Note:** If the tool had `action_risk = None` (undeclared) or `Some(r >= Medium)`, the
`ActionRisk` gate would deny WITHOUT calling the inner hook (per EC-009, {INV-004}); the
`PendingHumanApproval` path would never be reached for that tool.
...
**Scope note (node-body panics, CWE-248):** The `catch_unwind` wraps the ENTIRE
`runner.run(input, policy)` future, which spans `GraphRunner::run` →
`CompiledStateGraph::invoke` → graph node execution → `extract_output`. A panic in
a graph node body during `.await` polling (e.g., `panic!("node error")` inside a node
...
| TV-005 | `ForceApproveHooks` policy; tool has `action_risk = Some(ActionRisk::ReadOnly)` (gate approves, `ReadOnly < Medium`); `PreToolCallHook` returns `PendingHumanApproval` (overridden to `Approve`); later node calls `interrupt()` | `isError: true`, E-MCP-010 message — interrupt not suppressed by `ForceApproveHooks`; note: a tool with `action_risk = None` or `>= Medium` would instead be gate-denied per EC-009 (inner hook never called) | EC-006, {INV-002} |
...
| TV-011 | `extract_output = `\|`_`\|` panic!("boom")`; graph succeeds to terminal state; server receives `tools/call` | `{ "content": [{ "type": "text", "text": "internal error" }], "isError": true }`; a subsequent `tools/call` to a different (non-panicking) tool returns `isError: false` | extract_output panic recovery (EC-010); `FutureExt::catch_unwind(AssertUnwindSafe(...))` inside `invoke_dyn` catches panic during async future polling; containment scope: `AssertUnwindSafe(runner.run(...))` wraps the ENTIRE future — ALL panics during `.await` polling are caught, including graph-node-body panics (same static response; see EC-010 scope note); SEC-008: requires `panic = "unwind"` |
...
| TV-019 | Graph wraps a graph that includes a conditional edge whose path_fn panics (e.g., `panic!("routing error")` inside the path function); `tools/call` invokes the graph agent tool; `ConcreteGraphRunner::run` → `CompiledStateGraph::invoke` → conditional edge routing calls path_fn which panics | `{ "content": [{ "type": "text", "text": "internal error" }], "isError": true }` — path_fn panic is caught in the Pregel executor (std::panic::catch_unwind, synchronous per BC-2.02.005 {PRE-004}/{PC-005}); surfaces as Err(E-GRAPH-011); the E-GRAPH-011 internal-panic code triggers ADR-029 §Decision 5 static replacement by code-match at the MCP boundary: `content[0].text == "internal error"` (NOT the redacted E-GRAPH-011 message which contains `<source_node>` and `<message>` dynamic fields that would leak internal routing topology); `isError: true`; server continues serving | EC-003 internal-panic static-replace exception (F-P2A177-02); path_fn panic is caught in the Pregel executor (std::panic::catch_unwind, synchronous per BC-2.02.005 {PRE-004}/{PC-005}) and surfaces as Err(E-GRAPH-011); the E-GRAPH-011 internal-panic code triggers ADR-029 §Decision 5 static replacement by code-match at the MCP boundary: content[0].text = "internal error" (CWE-209 — dynamic fields source_node and panic message suppressed) |
