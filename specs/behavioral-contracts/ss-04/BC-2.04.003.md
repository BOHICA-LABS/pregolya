...
| EC-003 | A caller attempts to construct a checkpoint with a random UUID (`Uuid::new_v4()`) as the ID | Compile-time type mismatch or runtime `Err(PregolyaError { category: INTERNAL, code: E-CHKPT-002, message: "MonotonicClockRegression: checkpoint_id must be monotonic: random UUID rejected", .. })` |
...
| Attempt `CheckpointSaver::put` with checkpoint whose ID is `Uuid::new_v4().to_string()` | `Err(PregolyaError { category: INTERNAL, code: E-CHKPT-002, .. })` — non-monotonic ID rejected | error |
