...
`Err(PregolyaError { code: "E-TMPL-003", .. })` unconditionally — in both the f-string engine and
...
| TV-004 | `template = "Hello, {name}!"`, `vars = {}` (name absent) | `Err(PregolyaError { code: "E-TMPL-003", message: "UndefinedVariable: variable 'name' is not defined in the template context", .. })` | error-case (undefined variable) |
...
- `architecture/purity-boundary-map.md` — `pregolya-prompts / prompts::template` classification (Pure Core — no I/O)
...
| Module | pregolya-prompts / prompts::template |
| Priority | P1 |
| Wave | 2 |
